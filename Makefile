...
(										\
	echo $(LDFLAGS_zephyr); 						\
	echo "$(ROM_FILES) $(LINKFLAGPREFIX)-Map=$(KERNEL_NAME).map"; 			\
...
SDK_BUILD_TIME=$(shell LANG=C LC_ALL=C date)

define filechk_sdk_version.h
       (echo "#ifndef _SDK_VERSION_H_"; \
       echo "#define _SDK_VERSION_H_"; \
       echo ;\
       echo "#define SDK_BUILD_TIME             \"$(SDK_BUILD_TIME)\""; \
       echo "#define REVISION_INFO              \"$(BUILD_INFO)-$(USER_NAME)\""; \
       echo; \
...
datafs_bin:
	$(Q)if [ "$(CONFIG_DISK_ACCESS_NOR)" = "y" ] && [ -d  $(PROJECT_BASE)/app_conf/${APP_CONFIG}/datafs ]; then	\
		...
		dd if=/dev/zero of=$(FW_DIR)/bin/rodatafs.bin bs=512 count=1 2>&1; \
	fi;
...
sdfs_bin:
	...
	$(Q) if [ -f $(PROJECT_BASE)/app_conf/${APP_CONFIG}/sdfs/adMUSIC.dsp ]; then \
		echo "Use local DSP files"; \
	else \
		if [ -d "$(PROJECT_BASE)/dsp_bin/default" ]; then \
			echo "Copy default DSP files..."; \
			find "$(PROJECT_BASE)/dsp_bin/default" -type f -name '*.dsp' -exec cp {} "$(OUTDIR)/sdfs/" \; ; \
		fi; \
	fi;

	$(Q)if [ "$(CONFIG_HFP_SPEECH)" = "y" ]; then	\
		echo "Copy hfp.dsp"; \
		cp $(PROJECT_BASE)/dsp_bin/default/hfp.dsp $(OUTDIR)/sdfs/; \
	fi;

	$(Q)if [ "X$(CONFIG_AUDIO_MULTI_CHANNEL_MODE)" != "X" ] ; then \
		if [ -d $(PROJECT_BASE)/app_conf/${APP_CONFIG}/efx/m${CONFIG_AUDIO_MULTI_CHANNEL_MODE} ]; then \
			echo "Copy $(PROJECT_BASE)/app_conf/${APP_CONFIG}/efx/m${CONFIG_AUDIO_MULTI_CHANNEL_MODE}/*"; \
			cp -rf  $(PROJECT_BASE)/app_conf/${APP_CONFIG}/efx/m${CONFIG_AUDIO_MULTI_CHANNEL_MODE}/* ${OUTDIR}/sdfs/; \
		fi \
	fi;
...
# Args for build_firmware.py
BUILD_FIRMWARE_ARGS = -c $(FW_DIR)/firmware.xml \
			-e $(FW_DIR)/bin/encrypt.bin \
			-ef $(FW_DIR)/bin/efuse.bin \
			-s $(CONFIG_SOC_SERIES) \
			-b $(BOARD_NAME) \
			-a ${APP_CONFIG}
ifeq ($(CONFIG_SOC_SECURE_BOOT),y)
	BUILD_FIRMWARE_ARGS += -secure_boot 1
endif
ifeq ($(CONFIG_OTA),y)
	BUILD_FIRMWARE_ARGS += -ota 1
endif
ifeq ($(CONFIG_OTA_TEMP_BIN_NOT_ENCRYPT), y)
	BUILD_FIRMWARE_ARGS += -ota_encrypt_temp 0
else
	BUILD_FIRMWARE_ARGS += -ota_encrypt_temp 1
endif

BOARD_DIR=$(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)
firmware: nvram_bin datafs_bin sdfs_bin
	...
	$(Q)if [ -f $(BOARD_DIR)/fw_maker_ext.cfg ]; then 	\
		ls $(BOARD_DIR)/fw_maker_ext.cfg;		\
		cp $(BOARD_DIR)/fw_maker_ext.cfg $(FW_DIR)/bin/;\
	fi;
	...
	$(Q)if [ "$(CONFIG_OTA_RECOVERY)" = "y" ]; then	\
		...
		cp $(O)_recovery/$(KERNEL_BIN_NAME) $(FW_DIR)/bin/recovery.bin; \
	fi;

	$(Q)if [ -f $(O)/../$(BOARD_NAME)_second/$(KERNEL_BIN_NAME) ]; then	\
		echo 'FW: Copy zephyr_second.bin';		\
		cp $(O)_second/$(KERNEL_BIN_NAME) $(FW_DIR)/bin/zephyr2.bin; \
	fi
	...
	$(Q)if [ -f $(O)/firmware.xml ] ; then	\
		echo 'FW: Override the default firmware.xml';	\
		cp $(O)/firmware.xml $(FW_DIR)/;	\
	fi;

	$(Q)if [ -f $(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/efuse.bin ] ; then		\
		cp $(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/efuse.bin $(FW_DIR)/bin/;	\
	fi;

	$(Q)if [ "$(CONFIG_ACTIONS_ATT)" = "y" ]; then	\
		if [ -d $(FW_DIR)/att ]; then		\
			rm -rf $(FW_DIR)/att/*;		\
		else					\
			mkdir -p $(FW_DIR)/att;		\
		fi;					\
		if [ -d $(PROJECT_BASE)/att ]; then	\
			cp -r $(PROJECT_BASE)/att/* $(FW_DIR)/att/;	\
		fi; 					\
	fi;


	$(Q)if [ -f $(PROJECT_BASE)/app_conf/${APP_CONFIG}/public.pem ] ; then	\
		echo 'FW: Override the default public.pem';	\
		cp $(PROJECT_BASE)/app_conf/${APP_CONFIG}/public.pem $(FW_DIR)/;	\
	fi;

	$(Q)if [ -f $(PROJECT_BASE)/app_conf/${APP_CONFIG}/mbrec_${CONFIG_BOARD}.bin ] ; then	\
		echo 'FW: Override the default mbrec.bin';	\
		cp $(PROJECT_BASE)/app_conf/${APP_CONFIG}/mbrec_${CONFIG_BOARD}.bin $(FW_DIR)/bin/mbrec.bin;	\
	fi;

	$(Q)if [ -f $(PROJECT_BASE)/app_conf/${APP_CONFIG}/mp_usb_andes.bin ] ; then	\
		echo 'FW: Override the default mp_usb_andes.bin';	\
		cp $(PROJECT_BASE)/app_conf/${APP_CONFIG}/mp_usb_andes.bin $(FW_DIR)/bin;	\
	fi;

	$(PYTHON) -B $(srctree)/scripts/support/actions/build_firmware.py $(BUILD_FIRMWARE_ARGS)

update-media-library: $(MEDIA_LIB)
	@echo "media library is updated successfully $(MEDIA_LIB)"

$(MEDIA_LIB):
	@echo "gererate media library $(MEDIA_LIB_OUT_DIR)/$@.a"
	$(Q)mkdir -p $(MEDIA_LIB_OUT_DIR)
	$(Q)rm -f $(MEDIA_LIB_OUT_DIR)/$@.a
	$(Q)find $(MEDIA_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(MEDIA_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(MEDIA_LIB_OBJ_DIR)/$@.a $(MEDIA_LIB_OUT_DIR)/$@.a

update-ota-library: $(OTA_LIB)
	@echo "ota library is updated successfully $(OTA_LIB)"

$(OTA_LIB):
	@echo "gererate ota library $(OTA_LIB_OUT_DIR)/$@.a"
	$(Q)mkdir -p $(OTA_LIB_OUT_DIR)
	$(Q)rm -f $(OTA_LIB_OUT_DIR)/$@.a
	$(Q)find $(OTA_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(OTA_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(OTA_LIB_OBJ_DIR)/$@.a $(OTA_LIB_OUT_DIR)/$@.a

update-bt-service-library: $(BT_SERVICE_LIB)
	@echo "bt service library is updated successfully $(BT_SERVICE_LIB)"

$(BT_SERVICE_LIB):
	@echo "gererate bt service library $(BT_SERVICE_LIB_OUT_DIR)/$@.a"
	$(Q)rm -f $(BT_SERVICE_LIB_OUT_DIR)/$@.a
	$(Q)find $(BT_SERVICE_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(BT_SERVICE_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(BT_SERVICE_LIB_OBJ_DIR)/$@.a $(BT_SERVICE_LIB_OUT_DIR)/$@.a

update-bt-stack-library: $(BT_STACK_LIB)
	@echo "bt stack library is updated successfully $(BT_STACK_LIB)"

$(BT_STACK_LIB):
	@echo "gererate bt stack library $(BT_STACK_LIB_DIR)/$@.a"
	$(Q)rm -f $(BT_STACK_LIB_DIR)/$@.a
	$(Q)find $(BT_STACK_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(BT_STACK_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(BT_STACK_LIB_OBJ_DIR)/$@.a $(BT_STACK_LIB_DIR)/$@.a
...
update-mp-library: $(MP_LIB)
	@echo "mp library is updated successfully $(MP_LIB)"

$(MP_LIB):
	@echo "gererate mp library $(MP_LIB_OUT_DIR)/$@.a"
	$(Q)mkdir -p $(MP_LIB_OUT_DIR)
	$(Q)rm -f $(MP_LIB_OUT_DIR)/$@.a
	$(Q)find $(MP_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(MP_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(MP_LIB_OBJ_DIR)/$@.a $(MP_LIB_OUT_DIR)/$@.a

update-secureboot-library: $(SECUREBOOT_LIB)
	@echo "secureboot library is updated successfully $(SECUREBOOT_LIB)"

$(SECUREBOOT_LIB):
	@echo "gererate secureboot library $(SECUREBOOT_LIB_OUT_DIR)/$@.a"
	$(Q)mkdir -p $(SECUREBOOT_LIB_OUT_DIR)
	$(Q)rm -f $(SECUREBOOT_LIB_OUT_DIR)/$@.a
	$(Q)find $(SECUREBOOT_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(SECUREBOOT_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(SECUREBOOT_LIB_OBJ_DIR)/$@.a $(SECUREBOOT_LIB_OUT_DIR)/$@.a
