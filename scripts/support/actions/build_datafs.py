...
def parse_config(output_file, cfg_file):
    #print('FW: Parse config file: %s' %cfg_file)
    part_num = 0
    partitions = []
    bin_dir = os.path.dirname(output_file)
    bin_name = os.path.basename(output_file)
    ...
    for part in part_list:
        ...
        partitions.append(part_prop)
        part_num = part_num + 1

    part_num = len(partitions);
    ...
    for part in partitions:
        ...
        if bin_name == part['file_name']:
            print("DATAFS: partitions %s size 0x%x" % (part['name'], part_size))
            return part_size
    return 0
...
def main(argv):
    ...
    parser.add_argument('-c', dest = 'cfg_file', required=True)
    #parser.add_argument('-s', dest = 'image_size', required=True, type=lambda x: hex(int(x,0)))
    args = parser.parse_args();
    image_size = parse_config(args.output_file, args.cfg_file)

    #print('DATAFS: Build datafs image (fatfs) size 0x%x\n' %(image_size))
    if image_size == 0:
        sys.exit(0)

    # generate empty image file
    new_file(args.output_file, image_size, 0xff)
