...
#define ACT_LOG_MODEL_ID ALF_MODEL_SHELL
...
#ifdef CONFIG_SHELL_THREAD
#define STACKSIZE CONFIG_CONSOLE_SHELL_STACKSIZE
static K_THREAD_STACK_MEMBER(stack, STACKSIZE) __stack_noinit;
static struct k_thread shell_thread;
#else
static struct k_delayed_work shell_work;
#endif
...
static size_t line2argv(char *str, char *argv[], size_t size)
{
	...
	while ((str = strchr(str, ' '))) {
		*str++ = '\0';
		...
		if (argc == size) {
			ACT_LOG_ID_INF(ALF_STR_line2argv__TOO_MANY_PARAMETERS_, 1, size - 1);
			return 0;
		}
	}
...
static const char *get_command_and_module(char *argv[], int *module)
{
	*module = -1;
	...
	if (!argv[0]) {
		ACT_LOG_ID_INF(ALF_STR_get_command_and_module__UNRECOGNIZED_COMMAND, 0);
		return NULL;
	}
...
static void print_module_commands(const int module)
{
	...
	ACT_LOG_ID_INF(ALF_STR_print_module_commands__HELPN, 0);
...
static int show_help(int argc, char *argv[])
{
	...
	if ((argc == 2) || ((default_module != -1) && (argc == 1))) {
		...
		ACT_LOG_ID_INF(ALF_STR_show_help__NENTER_EXIT_TO_LEAVE, 0);
	} else { /* help for all entities */
		ACT_LOG_ID_INF(ALF_STR_show_help__AVAILABLE_MODULESN, 0);
		for (module = 0; module < NUM_OF_SHELL_ENTITIES; module++) {
			printk("%s\n", __shell_cmd_start[module].module_name);
		}
		ACT_LOG_ID_INF(ALF_STR_show_help__NTO_SELECT_A_MODULE_, 0);
	}
...
static shell_cmd_function_t get_cb(int *argc, char *argv[], int *module)
{
	...
	if (!first_string || first_string[0] == '\0') {
		ACT_LOG_ID_INF(ALF_STR_get_cb__ILLEGAL_PARAMETERN, 0);
		return NULL;
	}
	...
	if ((*argc == 1) && (default_module == -1)) {
		ACT_LOG_ID_INF(ALF_STR_get_cb__MISSING_PARAMETERN, 0);
		return NULL;
	}
...
{
	printk("Unrecognized command: %s\n", argv);
	ACT_LOG_ID_INF(ALF_STR_print_cmd_unknown__TYPE_HELP_FOR_LIST_O, 0);
}
...
#else
static void shell_work_routinue(struct k_work *work)
{
	struct console_input *cmd;
...
static u8_t completion(char *line, u8_t len)
{
	...
	for (i = command_len; i < common_chars; i++) {
		ACT_LOG_ID_INF(ALF_STR_completion__C, 1, first_match[i]);
		line[len++] = first_match[i];
	}

	/* for convenience add space after command */
	if (space) {
		ACT_LOG_ID_INF(ALF_STR_completion___, 0);
		line[len] = ' ';
	}
...
#ifndef CONFIG_SHELL_THREAD
static void line_end(void)
{
	k_delayed_work_submit(&shell_work, 0);
}
#endif
...
#ifdef CONFIG_SHELL_THREAD
	k_thread_create(&shell_thread, (k_thread_stack_t)stack, STACKSIZE, shell, NULL, NULL,
			NULL, CONFIG_CONSOLE_SHELL_PRIORITY, 0, K_NO_WAIT);

	/* Register serial console handler */
#ifdef CONFIG_UART_CONSOLE
	uart_register_input(&avail_queue, &cmds_queue, completion, NULL);
#endif

#else
	k_delayed_work_init(&shell_work, shell_work_routinue);

	/* Register serial console handler */
#ifdef CONFIG_UART_CONSOLE
	uart_register_input(&avail_queue, &cmds_queue, completion, line_end);
#endif
