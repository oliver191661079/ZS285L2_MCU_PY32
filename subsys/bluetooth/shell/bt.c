...
#include <bluetooth/avrcp_controller.h>
#endif
...
NET_BUF_POOL_DEFINE(sdp_client_pool, CONFIG_BT_MAX_BR_CONN,
...
#if defined(CONFIG_BT_A2DP)
static struct bt_a2dp_endpoint a2dp_endpoint;
static void *bt_a2dp_session_hdl;

static int cmd_bredr_a2dp_register(int argc, char *argv[])
{
	int ret;

	ret = bt_a2dp_register_endpoint(&a2dp_endpoint, BT_A2DP_AUDIO, BT_A2DP_SINK);
...
{
	printk("Switch to salve role\n");
	if (bt_conn_switch_role(default_conn, BT_CONN_ROLE_SLAVE)) {
		printk("bt_conn_switch_role failed\n");
	}
...
static int cmd_bredr_a2dp_connect(int argc, char *argv[])
{
	...
	printk("%s conn:%p\n", __func__, default_conn);
	a2dp_conn = bt_a2dp_connect(default_conn);

	if (a2dp_conn) {
		printk("Connect a2dp_conn:%p\n", a2dp_conn);
		return 0;
	} else {
		printk("cmd_bredr_a2dp_connect failed\n");
		return -1;
	}
}
...
{
	bt_a2dp_disconnect(bt_a2dp_session_hdl);
	return 0;
}

static void a2dp_connect_cb(void *session_hdl, u8_t session_type, struct bt_conn *conn)
{
	char addr[BT_ADDR_LE_STR_LEN];

	conn_addr_str(conn, addr, sizeof(addr));

	printk("%s: %p, type:%d, mac:%s\n", __func__, session_hdl, session_type, addr);
	bt_a2dp_session_hdl = session_hdl;
}

static void a2dp_disconnected_cb(void *session_hdl, u8_t session_type, struct bt_conn *conn)
{
	printk("%s: %p\n", __func__, session_hdl);
	bt_a2dp_session_hdl = NULL;
}

static void a2dp_media_handler_cb(void *session_hdl, struct net_buf *buf)
{
	static u8_t cnt = 0;
...
static int a2dp_media_state_req_cb(void *session_hdl, u8_t state, struct bt_conn *conn)
{
	printk("%s: %p\n", __func__, session_hdl);
	switch (state) {
		case BT_A2DP_MEDIA_STATE_OPEN:
			printk("BT_A2DP_MEDIA_STATE_OPEN\n");
			break;
		case BT_A2DP_MEDIA_STATE_START:
			printk("BT_A2DP_MEDIA_STATE_START\n");
			break;
		case BT_A2DP_MEDIA_STATE_CLOSE:
			printk("BT_A2DP_MEDIA_STATE_CLOSE\n");
			break;
		case BT_A2DP_MEDIA_STATE_SUSPEND:
			printk("BT_A2DP_MEDIA_STATE_SUSPEND\n");
			break;
		default:
			break;
	}
...
static void a2dp_media_codec_config_cb(struct bt_a2dp_media_sbc_codec *codec)
{
	printk("%s\n", __func__);
}
...
static struct bt_a2dp_app_cb a2dp_app_cb = {
	...
	.media_codec_config = a2dp_media_codec_config_cb,
};
...
{
	const char *action;
	void *session = bt_a2dp_session_hdl;

	if (argc < 2) {
		printk("Used: bt br-a2dp-pts discover/getcap/getallcap/xxx\n");
		return 0;
	}
	...
	if (!strcmp(action, "discover")) {
		return bt_a2dp_discover(session);
	} else if (!strcmp(action, "getcap")) {
		return bt_a2dp_get_capabilities(session);
	} else if (!strcmp(action, "getallcap")) {
		return bt_a2dp_get_all_capabilities(session);
	} else if (!strcmp(action, "setcfg")) {
		return bt_a2dp_set_configuration(session);
	} else if (!strcmp(action, "reconfig")) {
		return bt_a2dp_reconfig(session);
	} else if (!strcmp(action, "open")) {
		return bt_a2dp_open(session);
	} else if (!strcmp(action, "start")) {
		return bt_a2dp_start(session);
	} else if (!strcmp(action, "suspend")) {
		return bt_a2dp_suspend(session);
	} else if (!strcmp(action, "close")) {
		return bt_a2dp_close(session);
	} else if (!strcmp(action, "abort")) {
		return bt_a2dp_abort(session);
	} else if (!strcmp(action, "report")) {
		return bt_a2dp_delayreport(session, 0);
	} else if (!strcmp(action, "disconnectmedia")) {
		return bt_a2dp_disconnect_media_session(session);
	} else {
		printk("Unkown cmd:%s\n", action);
	}
...
#if defined(CONFIG_BT_AVRCP)
static void *avrcp_session_hdl = NULL;

static void avrcp_ctrl_connected_cb(void *session_hdl, struct bt_conn *conn)
{
	char addr[BT_ADDR_LE_STR_LEN];

	conn_addr_str(conn, addr, sizeof(addr));

	printk("Avrcp connected: %p, mac:%s\n", session_hdl, addr);
	avrcp_session_hdl = session_hdl;
}

static void avrcp_ctrl_disconnected_cb(void *session_hdl, struct bt_conn *conn)
{
	printk("Avrcp disconnected:%p\n", session_hdl);
	if (avrcp_session_hdl == session_hdl) {
		avrcp_session_hdl = NULL;
	} else {
		printk("Not the connect session:%p\n", avrcp_session_hdl);
	}
}

static void avrcp_ctrl_event_notify_cb(void *session_hdl, struct bt_conn *conn,
								u8_t event_id, u8_t status)
{
	/* event_id, status defined in avrcp.h */
	printk("Notify event:%d, status:%d\n", event_id, status);
}
...
static int cmd_bredr_avrcp_connect(int argc, char *argv[])
{
	...
	printk("%s conn:%p\n", __func__, default_conn);
	avrcp_conn = bt_avrcp_controller_connect(default_conn);

	if (avrcp_conn) {
		printk("Connect avrcp_conn:%p\n", avrcp_conn);
		return 0;
	} else {
		printk("cmd_bredr_avrcp_connect failed\n");
		return -1;
	}
}
...
{
	if (avrcp_session_hdl) {
		bt_avrcp_controller_disconnect(avrcp_session_hdl);
	}
...
static int cmd_bredr_avrcp_pass(int argc, char *argv[])
{
	...
	if (argc < 2) {
		printk("Used: bt br-avrcp-pass 0x44(AVRCP_OPERATION_ID_PLAY)\n");
		return 0;
	}

	opid = strtoul(argv[1], NULL, 16);

	printk("Pass through operation id:0x%x\n", opid);
	return bt_avrcp_controller_pass_through_cmd(avrcp_session_hdl, opid);
}
...
static int cmd_bredr_avrcp_pass_continue(int argc, char *argv[])
{
	...
	if (argc < 3) {
		printk("Used: bt br-avrcp-pass 0x49(AVRCP_OPERATION_ID_PLAY) start/stop\n");
		return 0;
	}
	...
	printk("Pass through operation id:0x%x\n", opid);
	return bt_avrcp_controller_pass_through_continue_cmd(avrcp_session_hdl, opid,
...
{
	const char *action;
	void *session = avrcp_session_hdl;

	if (argc < 2) {
		printk("Used: bt br-avrcp-pts getcap/notify\n");
		return 0;
	}
	...
	if (!strcmp(action, "getcap")) {
		return bt_avrcp_controller_get_capabilities(session);
	} else if (!strcmp(action, "notify")) {
		return bt_avrcp_controller_register_notification(session);
	} else if (!strcmp(action, "status")) {
		return bt_avrcp_controller_get_play_status(session);
	} else {
		printk("Unkown cmd:%s\n", action);
	}
...
static int cmd_bredr_start(int argc, char *argv[])
{
	int ret;
	...
	printk("bt br-pscan on\n");
	ret = bt_br_set_connectable(true);
	if (ret) {
		printk("bt br-pscan failed\n");
		return ret;
	}

	k_sleep(100);

	printk("bt br-iscan on\n");
	ret = bt_br_set_discoverable(true);
	if (ret) {
		printk("bt br-iscan failed\n");
		return ret;
	}

	k_sleep(100);

	printk("bt br-a2dp-register sink\n");
	ret = bt_a2dp_register_endpoint(&a2dp_endpoint, BT_A2DP_AUDIO, BT_A2DP_SINK);
	if (ret) {
		printk("bt br-a2dp-register failed\n");
		return ret;
	}

	bt_a2dp_register_cb(&a2dp_app_cb);

	k_sleep(100);
	printk("bt br-avrcp_controller_register_sdp\n");
	ret = bt_avrcp_controller_register_sdp();
	if (ret) {
		printk("bt br-avrcp_controller_register_sdp failed\n");
		return ret;
	}

	bt_avrcp_controller_register_cb(&avrcp_ctrl_cb);
...
static struct bt_gatt_attr *gatt_indicate_att = NULL;
...
#endif /* CONFIG_BT_RFCOMM */
#if defined(CONFIG_BT_A2DP)
	{ "br-start", cmd_bredr_start, "bt init/br-pscsn/br-iscan/br-a2dp-register" },
	{ "br-switch-role", cmd_bredr_switch_role, "switch to slave role" },
	{ "br-a2dp-connect", cmd_bredr_a2dp_connect, "Connect a2dp source" },
	{ "br-a2dp-disconnect", cmd_bredr_a2dp_disconnect, "Disonnect a2dp source" },
	{ "br-a2dp-register", cmd_bredr_a2dp_register, "register a2dp sink" },
#if defined(CONFIG_BT_PTS_TEST)
	{ "br-a2dp-pts", cmd_bredr_a2dp_pts, "a2dp pts test" },
#endif
#endif /* CONFIG_BT_A2DP */
#if defined(CONFIG_BT_AVRCP)
	{ "br-avrcp-connect", cmd_bredr_avrcp_connect, "Connect avrcp target" },
	{ "br-avrcp-disconnect", cmd_bredr_avrcp_disconnect, "Disonnect avrcp target" },
	{ "br-avrcp-pass", cmd_bredr_avrcp_pass, "Send avrcp pass command" },
	{ "br-avrcp-pass_continue", cmd_bredr_avrcp_pass_continue, "Send avrcp pass continue command" },
#if defined(CONFIG_BT_PTS_TEST)
	{ "br-avrcp-pts", cmd_bredr_avrcp_pts, "avrcp pts test" },
#endif
#endif
#endif
