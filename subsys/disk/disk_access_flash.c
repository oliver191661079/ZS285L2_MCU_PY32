...
static u32_t disk_flash_start = 0;
static u32_t disk_flash_volume_size = 0;
...
static off_t lba_to_address(u32_t sector_num)
{
	...
	__ASSERT(flash_addr < (disk_flash_start +
		disk_flash_volume_size), "FS bound error");
		     ...
		     u32_t sector_count)
{
	...
	limit = (disk_flash_start + disk_flash_volume_size) / SECTOR_SIZE;
...
static int flash_disk_access_ioctl(struct disk_info *disk, u8_t cmd, void *buff)
{
	...
	case DISK_IOCTL_GET_SECTOR_COUNT:
		*(u32_t *)buff = disk_flash_volume_size / SECTOR_SIZE;
		return 0;
	...
	case DISK_IOCTL_GET_DISK_SIZE:
		*(u32_t *)buff = disk_flash_volume_size;
		return 0;
...
static struct disk_info disk_flash = {
	.name		= "FLASH",
	.id		= 0,
	.flag		= 0,
...
static int flash_disk_init(struct device *dev)
{
	...
	const struct partition_entry * part = partition_get_part(PARTITION_FILE_ID_FAT_FLASH);
	if(!part)
	{
		printk("XML no FAT_FLASH info\n");
		#ifdef CONFIG_DISK_FLASH_START
		disk_flash_start = CONFIG_DISK_FLASH_START;
		#endif

		#ifdef CONFIG_DISK_VOLUME_SIZE
		disk_flash_volume_size = CONFIG_DISK_VOLUME_SIZE;
		#endif
	}else{
		disk_flash_start = part->offset;
		disk_flash_volume_size = part->size;
	}

	disk_flash.sector_offset	= disk_flash_start / SECTOR_SIZE;
	disk_flash.sector_cnt	= disk_flash_volume_size / SECTOR_SIZE;

	printk("disk_flash 0x%x 0x%x\n", disk_flash_start, disk_flash_volume_size);
