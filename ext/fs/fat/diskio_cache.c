...
#define DISKIO_TIMEOUT_MS 10
static char diskio_inited = 0;

char diskio_is_init(void)
{
	return diskio_inited;
}
...
static void _diskio_cache_thread_loop(void *p1, void *p2, void *p3)
{
	...
	while (!diskio_cache_ctx->terminal) {
		...
		cache_req = os_fifo_get(&diskio_cache.cache_req_fifo, DISKIO_TIMEOUT_MS);
		if (!cache_req) {
			continue;
		}
		os_mutex_lock(&diskio_cache_mutex, OS_FOREVER);
	...
	/**make sure sub stread exit first*/
	os_thread_priority_set(os_current_get(), -1);
	diskio_cache_ctx->terminaled = 1;
	SYS_LOG_INF("exit!");

}

int diskio_cache_init(void)
{
...
int diskio_cache_deinit(void)
{
	...
	diskio_inited = 0;
	//exit thread
	diskio_cache.terminal = 1;

	//wait until thread exit
	while (!diskio_cache.terminaled)
		os_sleep(1);
