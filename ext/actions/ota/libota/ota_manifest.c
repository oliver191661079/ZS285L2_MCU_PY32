...
static __ota_bss uint8_t ota_manifest_buffer[OTA_MANIFSET_FILE_MAX_SIZE + 1] __aligned(4);
			    ...
			    const char *file_name)
{
	...
	if (file_size > OTA_MANIFSET_FILE_MAX_SIZE) {
		SYS_LOG_INF("cfg file is too big %d", file_size);
		return -1;
	}
	...
	manifest_data = (char *)ota_manifest_buffer;
	//if (!manifest_data) {
