...
/**
 * @brief system pre init
 *
 * @details this rontine make system pre init .
...
u32_t system_setget_poweroff_time(u8_t set_get, u32_t newtime_ms);
u32_t system_setget_standby_time( u8_t set_get, u32_t newtime_ms);
u8_t  system_is_in_standby(void);

/* clear le nvram when poweroff or reboot */
int system_le_cleanup_nvram_on_poweroff(void);

/* clear le nvram when poweron */
