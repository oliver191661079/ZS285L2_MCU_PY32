...
static int _audio_is_ready(void *aps_monitor)
{
	...
	stream = audio_track_get_stream(audio_track);
...
static int _audio_trigger_start(void *aps_monitor)
{
	...
	audio_system_trigger_track_start(AUDIO_STREAM_SUBWOOFER);
	return 0;
}
...
static uint32_t _audio_get_samples_cnt(void *aps_monitor, uint8_t *audio_mode)
{
	...
	sample = hal_aout_channel_get_sample_cnt(audio_track->audio_handle);
...
static int _audio_get_error_state(void *aps_monitor)
{
	...
	return hal_aout_channel_check_fifo_underflow(audio_track->audio_handle);
}
...
void audio_aps_monitor_tws_init(aps_monitor_info_t *handle,void *tws_observer)
{
	...
	//audio_aps_monitor_set_aps(audio_track->audio_handle, APS_OPR_FAST_SET, handle->aps_default_level);
}


void audio_aps_tws_notify_decode_err(aps_monitor_info_t *handle,uint16_t err_cnt)
{
...
void audio_aps_monitor_tws_deinit(aps_monitor_info_t *handle,void *tws_observer)
{
	...
	if (tws_observer) {
		hal_aout_channel_enable_sample_cnt(audio_track->audio_handle, false);
...
void audio_aps_monitor_master(aps_monitor_info_t *handle, int stream_length, uint8_t aps_max_level, uint8_t aps_min_level, uint8_t aps_level)
{
	...
	struct audio_track_t *audio_track = handle->output_handle;
...
void audio_aps_monitor_slave(aps_monitor_info_t *handle, int stream_length, uint8_t aps_max_level, uint8_t aps_min_level, uint8_t slave_aps_level)
{
	...
	audio_track = handle->output_handle;
