 ...
 *  @kconfig{CONFIG_BT_PROPERTY_CCC_LAZY_LOADING} or @kconfig{CONFIG_BT_CONN} is set.
 *
 *  @kconfig{CONFIG_BT_PROPERTY_CCC_LAZY_LOADING} will set BT_GATT_CCC_MAX to
 ...
 *
 */
#if defined(CONFIG_BT_PROPERTY_CCC_LAZY_LOADING)
#if SUPPORT_GATT_OVER_BR
	#define BT_GATT_CCC_MAX CONFIG_BT_MAX_CONN
#else
	#define BT_GATT_CCC_MAX (CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN)
#endif
...
struct _bt_gatt_ccc {
	 ...
	 */
	void (*cfg_changed)(struct bt_conn *conn, uint8_t conn_type,
	 ...
	 */
	ssize_t (*cfg_write)(struct bt_conn *conn, uint8_t conn_type,
			     const struct bt_gatt_attr *attr, uint16_t value);
 ...
 *  @param _perm CCC access permissions.
 */
#define BT_GATT_CCC_CB(_changed, _match, _write, _perm)				\
	BT_GATT_CCC_MANAGED(((struct _bt_gatt_ccc[])			\
		{BT_GATT_CCC_INITIALIZER(_changed, _write, _match)}), _perm)
