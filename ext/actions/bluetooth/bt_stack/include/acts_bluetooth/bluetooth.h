...
enum {
	...
	BT_SUPPORT_A2DP_SOURCE_BIT = BIT(16),
...
#ifdef CONFIG_BT_A2DP_SRC
#define BT_SUPPORT_A2DP_SRC_CAB BT_SUPPORT_A2DP_SOURCE_BIT
#else
#define BT_SUPPORT_A2DP_SRC_CAB 0
#endif
...
/* box UUID: 0xFDDF */
#define BOX_SERIVCE_UUID		0xfddf
/*
Product ID list
  0x20E4 - VM GO4
  0x210A - Actions GO4
*/
#define BOX_PRODUCT_ID	CONFIG_BT_PRODUCT_ID

#define BOX_DEFAULT_MODEL_ID	0x01
#define BOX_DEFAULT_COLOR_ID	0x01
#endif

/* Actions add end */

/* advertiser identity address type */
enum {
	BT_ID_PUBLIC = 0x00,        /** For specifying the public identity. */
	BT_ID_STATIC_RANDOM = 0x01, /** For specifying the static random identity. */
};
...
#define BT_ID_DEFAULT BT_ID_STATIC_RANDOM
#else
#define BT_ID_DEFAULT BT_ID_PUBLIC
#endif
...
enum {
	...
	/**
	 * @brief Advertising to privacy-enabled peer.
	 *
	 * Enable use of Undirect Resolvable Private Address (RPA) as the target address
	 * in advertisements.
	 * This is required if the remote device is privacy-enabled and
	 * supports address resolution of the target address advertisement.
	 * If include #BT_LE_ADV_OPT_USE_IDENTITY, it will use identity address.
	 * The identity type can be determined by op->id, 0 -- public, 1 -- static random
	 */
	BT_LE_ADV_OPT_ADDR_RPA = BIT(19),
};
...
// if change,need update lib.
struct btsrv_broadcast_info {
	...
	/*
	 * append '\0' is much more readable, especially when used to
	 * report to upper layer!
	 */
	uint8_t local_name[32 + 1];
	uint8_t broadcast_name[32 + 1];
	uint8_t program_info[32 + 1];
	//  TODO: choose if sink support enc/std/high?!
	uint8_t encryption : 1;
	uint8_t quality_standard : 1;
	uint8_t quality_high : 1;

	/* Public Broadcast Announcements valid */
	uint8_t public_valid : 1;

	//  TODO: support language/contexts in PBP metadata ???
	uint8_t meta_len;
	uint16_t contexts;
	uint32_t language;

	sys_snode_t node;	/* linked to btsrv_broadcast_scanner */
};

typedef int (*bt_broadcast_scan_filter_cb)(struct btsrv_broadcast_info *info);
...
/**
 * @brief get the all device that paired from paired list.
 *
 * @param dev_buf the buffer to store the device, it is a array.
 * @param buf_count the buffer number.
 *
 * @return the device stored in dev_buf.
 */
uint8_t bt_le_get_paired_devices(bt_addr_le_t dev_buf[], uint8_t buf_count);


/**
 * @brief get the all device  from paired list.
 *
 * @param dev_buf the buffer to store the device, it is a array.
 * @param buf_count the buffer number.
 *
 * @return the device stored in dev_buf.
 */
uint8_t bt_le_get_devices(bt_addr_le_t dev_buf[], uint8_t buf_count);


/**
 * @brief get the all device as the type that paired from paired list.
 *
 * @param addr_type the device address type.
 * @param dev_buf the buffer to store the device, it is a array
 * @param buf_count the buffer number.
 *
 * @return the device stored in dev_buf.
...
/**
 * @brief Get the last paired device.
 *
 * @param dev buffer to store device.
 *
...
/**
 * @brief clear the paired device.
 *
 * @param dev the device's paired info will be cleaned.
 *
 * @return true is success, false is failed.
 */
bool bt_le_clear_device(bt_addr_le_t* dev);

bool bt_le_check_remote_cis_feature(struct bt_conn *conn);

bool bt_le_check_remote_bis_feature(struct bt_conn *conn);

#if defined(CONFIG_BT_PRIVACY)
/**
 * @brief enable/disable the address resolution in the controller, after the new device paired it will enable auto.
 *
 * @param enable #BT_HCI_ADDR_RES_ENABLE, #BT_HCI_ADDR_RES_DISABLE
 *
 * @return 0 is success, otherwise is failed.
 */
int bt_le_address_resolution_enable(uint8_t enable);

/**
 * @brief set the length of time the Controller uses a Resolvable Private Address before a new
 *  resolvable private address is generated and starts being used.
 *
 * @param timeout applies to all resolvable private addresses generated by the Controller.
 *        measured in seconds, range is 0x0001 to 0x0E10 (1s to 1hour).
 *
 * @return 0 is success, otherwise is failed.
 */
int bt_le_set_rpa_timeout(uint16_t timeout);
#endif
...
enum {
	...
	BT_CB_EVT_VS_PA_SEND_COMPLETE                  = 0xFF40,
};
...
struct bt_cb_evt_vs_send_per_data {
    uint8_t adv_handle;
    uint8_t adv_len;
	uint8_t *adv_data;
};
...
void bt_set_pts_mode(uint8_t enable);

uint8_t bt_get_pts_mode(void);

typedef void(*pnp_sdp_complete_cb)(struct bt_conn *conn,uint16_t vendor_id,uint16_t product_id);

int bt_sdp_pnp(struct bt_conn *conn,pnp_sdp_complete_cb cb);

int8_t bt_get_fixed_chan_default_status(void);
/** @brief CSB interface */
...
int bt_vs_read_link_info(struct bt_conn *conn, complete_event_cb cb);
int bt_vs_link_channel_set(struct bt_conn *conn, uint8_t *map);
...
int bt_sc_enable(uint8_t enable);

int bt_ctkd_enable(uint8_t enable);
