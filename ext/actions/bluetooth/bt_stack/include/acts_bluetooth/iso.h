...
/** Value to set the ISO data path over Callback. */
#define BT_ISO_DATA_PATH_CB         0xFE

/** Minimum interval value (N * 1.25 ms) */
#define BT_ISO_INTERVAL_MIN         0x0004
/** Minimum interval value (N * 1.25 ms) */
#define BT_ISO_INTERVAL_MAX         0x0C80
...
struct bt_iso_bis_params {
	uint32_t sync_delay;	// TODO: for broadcaster only??
	uint32_t latency;
	uint8_t phy;	// TODO: for broadcaster only??
	uint8_t nse;
	uint8_t bn;
	uint8_t pto;
	uint8_t irc;
...
struct bt_iso_big_create_param {
	...
	/** @brief Channel SDU interval in us.
	 *
	 *  Value range BT_ISO_SDU_INTERVAL_MIN - BT_ISO_SDU_INTERVAL_MIN.
	 */
	uint32_t interval;
...
struct bt_iso_big_create_test_param {
	/** Array of pointers to BIS channels */
	struct bt_iso_chan **bis_channels;
	...
	/** @brief Channel ISO interval in us.
	 *
...
/** @brief Creates a BIG (Test) as a broadcaster
 *
 *  @param[in] padv      Pointer to the periodic advertising object the BIGInfo shall be sent on.
 *  @param[in] param     The parameters used to create and enable the BIG. The QOS parameters are
 *                       determined by the QOS field of the first BIS in the BIS list of this
