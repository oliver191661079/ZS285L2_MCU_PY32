...
#if defined(CONFIG_BT_GATT_OVER_BR)
#define SUPPORT_GATT_OVER_BR		1
#else
#define SUPPORT_GATT_OVER_BR		0
#endif /* CONFIG_BT_GATT_OVER_BR */

/* Need a name different than bt_gatt_over_br_user_info for a different section */
struct bt_gatt_over_br_user_info {
	int (*user_cb)(struct bt_conn *conn, uint8_t connected);
};

#define BT_GATT_OVER_BR_USER_INFO_DEFINE(_name, _cb)		\
	const Z_STRUCT_SECTION_ITERABLE(bt_gatt_over_br_user_info, _name) = { \
				.user_cb = _cb,		\
			}
