...
#define BT_LE_DLE_MAX_LEN	258//247	/* 251 - 4, LE DLE max length(251) */
#define L2CAP_MAX_LE_MPS	MIN(CONFIG_BT_L2CAP_RX_MTU, BT_LE_DLE_MAX_LEN)
#else
...
__in_section_unique(BT_MANAGER_BSS) static struct bt_l2cap bt_l2cap_pool[CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN];
...
void bt_l2cap_connected(struct bt_conn *conn)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR) &&
	    (conn->type == BT_CONN_TYPE_BR || conn->type == BT_CONN_TYPE_BR_SNOOP)) {
...
static void l2cap_chan_send_req(struct bt_l2cap_chan *chan,
				struct net_buf *buf, int timeout)
{
...
struct net_buf *bt_l2cap_create_pdu_timeout_len(struct net_buf_pool *pool,
					    ...
					    int timeout)
{
	return bt_conn_create_pdu_timeout_len(pool,
					  sizeof(struct bt_l2cap_hdr) + reserve, len,
					  timeout);
}
...
static void l2cap_chan_ready(struct bt_l2cap_chan *chan)
{
	struct bt_l2cap_le_chan *ch;

	if (!chan || chan->state != BT_L2CAP_CONNECTED) {
		return;
	}

	ch = BT_L2CAP_LE_CHAN(chan);

	l2cap_chan_tx_give_credits(ch, ch->tx.init_credits);
...
static struct net_buf *l2cap_alloc_frag(int timeout, void *user_data)
{
...
void bt_l2cap_profile_init(void)
{
	...
	extern int bt_gatt_over_br_init(void);
	bt_gatt_over_br_init();
#endif

#ifdef CONFIG_BT_FPMS
	extern int bt_fpms_init(void);
	bt_fpms_init();
#endif
