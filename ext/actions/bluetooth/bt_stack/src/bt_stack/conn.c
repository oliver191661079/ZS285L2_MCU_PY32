...
extern struct bt_conn acl_conns[CONFIG_BT_MAX_CONN];
extern struct bt_conn sco_conns[CONFIG_BT_MAX_BR_CONN];
extern struct bt_conn acl_le_adv_conn;
extern struct net_data_pool host_tx_pool;

#define SNIFF_ENTER_EXIT_TIMEOUT		1100		/* Enter sniff or exit sniff timeout */
#define SNIFF_MAX_INTERVAL		800		/* 800*0.625ms = 500ms */
#define SNIFF_MIN_INTERVAL		800		/* 800*0.625ms = 500ms */
#define SNIFF_ATTEMPT			4
#define SNIFF_TIMEOUT			1
#define SNIFF_WORK_INTERVAL		K_MSEC(1000)	/* 1000ms */
...
//static uint8_t bt_conn_enable_sniff_check;
//static struct k_delayed_work sniff_mode_work;
...
BT_BUF_POOL_DEFINE(tws_media_tx_pool, 4,
			BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU),
			sizeof(struct tx_meta), NULL, &host_tx_pool);

BT_BUF_POOL_DEFINE(tws_sync_tx_pool, 10,
			BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU),
			sizeof(struct tx_meta), NULL, &host_tx_pool);
...
static void notify_connected(struct bt_conn *conn)
{
	...
	if (conn->type == BT_CONN_TYPE_BR || conn->type == BT_CONN_TYPE_BR_SNOOP) {
		conn->br.mode = 0;
...
//		bt_spp_conn_disconnect_notify(conn);
	}
}
...
bool notify_remote_linkkey_miss(struct bt_conn *conn)
{
	struct bt_conn_cb *cb;
	bool ret = true;
	printk("notify_remote_linkkey_miss\n");

	SYS_SLIST_FOR_EACH_CONTAINER(&conn_cbs, cb, node) {
		if (conn->type == BT_CONN_TYPE_BR) {
			if (cb->remote_linkkey_miss) {
				ret =  cb->remote_linkkey_miss(conn,&conn->br.dst);
			}
		}else if(conn->type == BT_CONN_TYPE_LE){
			if (cb->remote_linkkey_miss) {
				ret = cb->remote_linkkey_miss(conn,&conn->le.dst.a);
			}
		}
		if(!ret){
			return ret;
...
{
	return bt_conn_new(acl_conns, bt_inner_value.max_conn);
}
...
struct bt_conn *bt_conn_lookup_addr_br_snoop(const bt_addr_t *peer)
{
	...
	for (i = 0; i < bt_inner_value.max_conn; i++) {
		...
		if (conn->type != BT_CONN_TYPE_BR_SNOOP) {
			bt_conn_unref(conn);
			continue;
		}
...
int bt_conn_set_security(struct bt_conn *conn, bt_security_t sec)
{
	...
	if (IS_ENABLED(CONFIG_BT_BR_SC_ONLY_MODE)) {
		sec = BT_SECURITY_L4;
	}
	...
	if ((IS_ENABLED(CONFIG_BT_BR_SC_ONLY_MODE) || (IS_ENABLED(CONFIG_BT_BR_SC_HOST_SUPP) && sec == BT_SECURITY_L4)) &&
		conn->encrypt > 0){
		BT_WARN("Security enabled, sec_level: %d, sec: %d, req_level: %d", conn->sec_level, sec, conn->required_sec_level);
		return -ENOTBLK;
	}
...
{
	sys_slist_append(&conn_cbs, &cb->node);
}

void bt_conn_cb_unregister(struct bt_conn_cb *cb)
{
	sys_slist_find_and_remove(&conn_cbs, &cb->node);
}
...
static void conn_cleanup_tx_queue(struct bt_conn *conn)
{
	struct net_buf *buf;
...
void bt_conn_process_tx(struct bt_conn *conn)
{
	...
	if (conn->type == BT_CONN_TYPE_BR_SNOOP) {
		BT_ERR("Send acl in snoop link!\n");
//		stack_print_hex("Error tx snoop buf", buf->data, MIN(buf->len, 32));
		net_buf_unref(buf);
		return;
	}
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
	...
	case BT_CONN_CONNECTED:
		/*
		 * The disconnect process is split into two parts, the prio
		 * part may race with connected event, check old state first
		 * to avoid the race condition.
		 */
		if (old_state == BT_CONN_DISCONNECT_COMPLETE) {
			/* revert state */
			conn->state = old_state;
			BT_WARN("revert state");
			return;
		}

		if (conn->type == BT_CONN_TYPE_SCO ||
			conn->type == BT_CONN_TYPE_SCO_SNOOP) {
...
void bt_conn_le_adv_update(struct bt_conn *conn)
{
    if(conn){
        memcpy(&acl_le_adv_conn,conn,sizeof(struct bt_conn));
    }
    else{
        memset(&acl_le_adv_conn,0,sizeof(struct bt_conn));
    }
}
					...
					const bt_conn_state_t state)
{
    ...
    if (acl_le_adv_conn.type == BT_CONN_TYPE_LE){
        if ((acl_le_adv_conn.state == state) && (acl_le_adv_conn.id == id)){
            conn = bt_conn_add_le(id,peer);
            if(conn){
...
{
	int ret = 0;
	struct bt_conn *conn = bt_conn_lookup_handle(handle);
	if (conn) {
...
#else
struct net_buf *bt_conn_create_frag_timeout(size_t reserve, int timeout)
#endif
{
...
{
	struct net_buf *buf;
	size_t ext_reserve = sizeof(struct bt_hci_acl_hdr) + BT_BUF_RESERVE;
	size_t alloc_len = ext_reserve + reserve + len;
	...
	if (IS_ENABLED(CONFIG_BT_DEBUG_CONN)) {
#if defined(CONFIG_NET_BUF_LOG)
		buf = net_buf_alloc_fixed_len_debug(pool, alloc_len, K_NO_WAIT, func, line);
#else
		buf = net_buf_alloc_len(pool, alloc_len, K_NO_WAIT);
#endif
		if (!buf) {
			BT_WARN("Unable to allocate buffer with K_NO_WAIT");
#if defined(CONFIG_NET_BUF_LOG)
			buf = net_buf_alloc_fixed_len_debug(pool, alloc_len, timeout, func,
							line);
#else
			buf = net_buf_alloc_len(pool, alloc_len, timeout);
#endif
		}
	} else {
#if defined(CONFIG_NET_BUF_LOG)
...
int bt_conn_qos_setup(struct bt_conn_qos_param *qos)
{
    struct bt_hci_cp_qos_setup *cp;
    struct net_buf *buf;
    struct bt_conn *conn = qos->conn;

    buf = bt_hci_cmd_create(BT_HCI_OP_QOS_SETUP, sizeof(*cp));
    if (!buf) {
        return -ENOMEM;
    }

	cp = net_buf_add(buf, sizeof(*cp));
	cp->handle = sys_cpu_to_le16(qos->conn->handle);

    cp->flags = 0;
    cp->service_type = qos->service_type;
    cp->token_rate = qos->token_rate;
    cp->latency = qos->latency;
    cp->peak_bandwidth = qos->peak_bandwidth;
    cp->delay_variation = qos->delay_variation;

    BT_DBG("conn:%d type:%d rate:%d latency:%d peak:%d delay:%d",
        (uint32_t)conn,
        cp->service_type,
        cp->token_rate,
        cp->latency,
    ...
    return bt_hci_cmd_send(BT_HCI_OP_QOS_SETUP, buf);
}
...
void* bt_conn_alloc_act_fix_cid_netbuf(uint16_t len,k_timeout_t timeout)
{
	struct net_buf *buf = NULL;

	buf = bt_l2cap_create_pdu_timeout_len(&tws_sync_tx_pool, 0, len, timeout);
	if (!buf) {
		return NULL;
	}
...
int bt_conn_send_act_fix_cid_data_with_netbuf(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len, void*netbuf)
{
	struct net_buf *buf = netbuf;
	...
	if (!bt_l2cap_is_br_act_fix_cid(cid)) {
		return -EIO;
	}
...
int bt_conn_send_act_fix_cid_data(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len)
{
	...
	buf = bt_l2cap_create_pdu_len(&tws_sync_tx_pool, 0, len);
	if (!buf) {
		return -ENOMEM;
	}
...
int bt_conn_send_act_fix_cid_data_by_media(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len, k_timeout_t timeout)
{
	struct net_buf *buf;
	...
	buf = bt_l2cap_create_pdu_timeout_len(&tws_media_tx_pool, 0, len, timeout);
	if (!buf) {
		BT_WARN("TWS media allocate buffer failure");
		return -ENOMEM;
	}
...
void bt_conn_notify_connectionless_data(struct bt_conn *conn, uint16_t cid, uint8_t *data, uint16_t len)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&conn_cbs, cb, node) {
		if (cb->rx_act_fix_cid_data) {
			cb->rx_act_fix_cid_data(conn, cid, data, len);
		}
	}
}
...
static uint16_t bt_conn_br_tx_pending_cnt(struct bt_conn *conn)
{
	uint16_t cnt = 0;
	sys_snode_t *node;
	struct bt_conn_tx *tx;
...
static int bt_hci_disconnect_async(uint16_t handle, uint8_t reason)
{
	struct net_buf *buf;
...
void bt_conn_reset_all_br(void)
{
	int i;
	...
	for (i = 0; i < bt_inner_value.max_conn; i++) {
		...
		if ((conn->state == BT_CONN_CONNECTED) && (conn->type == BT_CONN_TYPE_BR)) {
			conn_cleanup_tx_queue(conn);
			int err = bt_hci_disconnect_async(bt_conn_get_handle(conn),
...
__ramfunc uint8_t bt_conn_get_role(const struct bt_conn *conn)
{
	if(conn)
		return conn->role;
	else
...
{
	if (bt_conn_get_type(conn) == BT_CONN_TYPE_SCO) {
		return conn->sco.acl->handle;
	}
...
struct bt_conn * bt_conn_get_conn_by_handle(uint16_t handle)
{
	struct bt_conn *conn = bt_conn_lookup_handle(handle);
	if (conn) {
		bt_conn_unref(conn);
...
int bt_conn_set_snoop_media_rx_cid(struct bt_conn *conn, uint16_t cid)
{
	...
	conn->snoop_a2dp_media_rx_cid = cid;
	return 0;
}
...
bool bt_conn_is_snoop_media_rx_cid(uint16_t handle, uint16_t cid)
{
	...
	if (conn->type == BT_CONN_TYPE_BR_SNOOP) {
		//if (conn->snoop_a2dp_media_rx_cid == cid)
			ret = true;
	}
	bt_conn_unref(conn);
...
int bt_conn_set_snoop_switch_lock(struct bt_conn *conn, uint8_t lock)
{
	...
	conn->snoop_switch_lock = lock ? 1 : 0;

	if (conn->snoop_switch_lock == 0) {
		while ((buf = net_buf_slist_get(&conn->switch_rx_pending))) {
			if (conn->type == BT_CONN_TYPE_BR) {
				printk("Add pending_rx %d\n", buf->len);
...
int bt_conn_is_le_switch_connected(struct bt_conn *conn)
{
	...
	return conn->le_switch_connected;
}
