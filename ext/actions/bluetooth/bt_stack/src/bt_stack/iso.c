...
BT_BUF_POOL_DEFINE(iso_rx_pool, CONFIG_BT_ISO_RX_BUF_COUNT,
			      CONFIG_BT_ISO_RX_MTU, 0, NULL, &host_rx_pool);
...
BT_BUF_POOL_DEFINE(iso_tx_pool, CONFIG_BT_ISO_TX_BUF_COUNT,
			      BT_ISO_SDU_BUF_SIZE(CONFIG_BT_ISO_TX_MTU),
			      0, NULL, &host_tx_pool);

#if CONFIG_BT_ISO_TX_FRAG_COUNT > 0
BT_BUF_POOL_DEFINE(iso_frag_pool, CONFIG_BT_ISO_TX_FRAG_COUNT,
			      BT_ISO_SDU_BUF_SIZE(CONFIG_BT_ISO_TX_MTU),
			      0, NULL, &host_tx_pool);
...
static uint16_t iso_handle = 0;
#endif

/* TODO: Allow more than one server? */
#if defined(CONFIG_BT_ISO_UNICAST)
#ifndef CONFIG_BT_ISO_CIG_ID_START
#define CONFIG_BT_ISO_CIG_ID_START	0
#endif
...
static __unused void bt_iso_send_cb(struct bt_conn *iso, void *user_data)
{
...
struct net_buf *bt_iso_get_rx_len(k_timeout_t timeout, int len)
{
	struct net_buf *buf = net_buf_alloc_len(&iso_rx_pool, len, timeout);
...
#if defined(CONFIG_BT_ISO_BROADCAST)
		} else if (iso->iso.type == BT_ISO_CHAN_TYPE_BROADCASTER ||
		    iso->iso.type == BT_ISO_CHAN_TYPE_SYNC_RECEIVER) {
				 ...
				 const char *func, int line)
{
	...
	BT_DBG("%s -> %s", bt_iso_chan_state_str(chan->state),
...
#else
	return bt_conn_send_direct(chan->iso, buf);
#endif
}
...
void hci_le_vs_cis_rx_features(struct net_buf *buf)
{
	struct bt_hci_evt_le_vs_cis_rx_features *evt = (void *)buf->data;
	...
	cis = iso_chan(iso);
	if (!cis || !cis->cis_params) {
		BT_ERR("Not found");
		bt_conn_unref(iso);
		return;
	}

	cis_params = cis->cis_params;
	cis_params->no_ack = evt->no_ack;
	cis_params->rx_delay = sys_get_le24(evt->rx_delay);

	BT_DBG("no_ack: %d, rx_delay: %d", cis_params->no_ack,
		cis_params->rx_delay);

	bt_conn_unref(iso);
}

static void set_cis_params(struct bt_conn *iso,
				struct bt_hci_evt_le_cis_established *evt)
{
	...
	cis = iso_chan(iso);
	if (cis == NULL) {
		BT_ERR("Could not lookup cis from ISO");
		return;
	}
	...
	cis_params->cig_sync_delay = sys_get_le24(evt->cig_sync_delay);
	cis_params->cis_sync_delay = sys_get_le24(evt->cis_sync_delay);
	cis_params->m_latency = sys_get_le24(evt->c_latency);
	cis_params->s_latency = sys_get_le24(evt->p_latency);
	cis_params->m_phy = evt->c_phy;
	cis_params->s_phy = evt->p_phy;
	cis_params->nse = evt->nse;
	cis_params->m_bn = evt->c_bn;
	cis_params->s_bn = evt->p_bn;
	cis_params->m_ft = evt->c_ft;
	cis_params->s_ft = evt->p_ft;
	cis_params->m_max_pdu = sys_le16_to_cpu(evt->c_max_pdu);
	cis_params->s_max_pdu = sys_le16_to_cpu(evt->p_max_pdu);
	cis_params->interval = sys_le16_to_cpu(evt->interval);
}
...
static struct bt_iso_cig *get_cig(const struct bt_iso_chan *iso_chan)
{
	...
	cig_id = iso_chan->iso->iso.cig_id - CONFIG_BT_ISO_CIG_ID_START;

	__ASSERT(cig_id < ARRAY_SIZE(cigs), "Invalid cig_id %u", cig_id);
...
static struct bt_iso_cig *get_free_cig(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(cigs); i++) {
		if (cigs[i].state == BT_ISO_CIG_STATE_IDLE) {
			...
			cigs[i].id = i + CONFIG_BT_ISO_CIG_ID_START;
...
int bt_iso_cig_terminate(struct bt_iso_cig *cig)
{
	...
	if (cig->state != BT_ISO_CIG_STATE_INACTIVE &&
		...
		BT_ERR("Invalid CIG state: %u", cig->state);
		return -EINVAL;
	}
...
int bt_iso_chan_connect(const struct bt_iso_connect_param *param, size_t count)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		if (param[i].iso_chan->state != BT_ISO_DISCONNECTED) {
			BT_WARN("[%zu]: ISO is not in the BT_ISO_DISCONNECTED state: %u",
			       i, param[i].iso_chan->state);
			/* FIXME: set to BT_ISO_DISCONNECTED, any better way? */
			param[i].iso_chan->state = BT_ISO_DISCONNECTED;
			return -EINVAL;
		}
	}

	/* Set connection states before creating CIS */
	for (size_t i = 0; i < count; i++) {
	...
	if (err) {
		/* revert connection states if failed to create CIS */
		for (size_t i = 0; i < count; i++) {
			bt_conn_set_state(param[i].iso_chan->iso,
					  BT_CONN_DISCONNECTED);
		}
		BT_WARN("Failed to connect CISes: %d", err);
		return err;
	}
			     ...
			     struct bt_iso_big_create_param *param)
{
	...
	/* Set connection states before creating BIG */
	SYS_SLIST_FOR_EACH_CONTAINER(&big->bis_channels, bis, node) {
	...
	if (err) {
		/* revert connection states if failed to create BIG */
		SYS_SLIST_FOR_EACH_CONTAINER(&big->bis_channels, bis, node) {
			bt_iso_chan_set_state(bis, BT_ISO_DISCONNECTED);
		}
		return err;
	}
		      ...
		      struct bt_iso_big **out_big)
{
	...
	CHECKIF(param->interval < BT_ISO_SDU_INTERVAL_MIN ||
		param->interval > BT_ISO_SDU_INTERVAL_MAX) {
		BT_DBG("Invalid sdu interval: %u", param->interval);
		return -EINVAL;
	}
...
static int hci_le_create_big_test(struct bt_le_ext_adv *padv, struct bt_iso_big *big,
			     struct bt_iso_big_create_test_param *param)
{
	...
	req->nse = param->nse;
	req->max_pdu = sys_cpu_to_le16(param->max_pdu);
	req->bn = param->bn;
	req->irc = param->irc;
	req->pto = param->pto;
...
int bt_iso_big_create_test(struct bt_le_ext_adv *padv,
				struct bt_iso_big_create_test_param *param,
				struct bt_iso_big **out_big)
{
	int err;
	...
	CHECKIF(param->iso_interval < BT_ISO_INTERVAL_MIN ||
		param->iso_interval > BT_ISO_INTERVAL_MAX) {
		BT_DBG("Invalid iso interval: %u", param->iso_interval);
		return -EINVAL;
	}
...
static void store_bis_broadcaster_info(const struct bt_hci_evt_le_big_complete *evt,
				       struct bt_iso_chan *bis)
{
	struct bt_iso_bis_params *bis_params = bis->bis_params;

	bis_params->interval = sys_le16_to_cpu(evt->iso_interval);
	bis_params->nse = evt->nse;

	bis_params->sync_delay = sys_get_le24(evt->sync_delay);
	bis_params->latency = sys_get_le24(evt->latency);
	bis_params->phy = bt_get_phy(evt->phy);
	bis_params->bn = evt->bn;
	bis_params->irc = evt->irc;
	bis_params->pto = evt->pto;
	bis_params->max_pdu = sys_le16_to_cpu(evt->max_pdu);
}
...
void hci_le_big_complete(struct net_buf *buf)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&big->bis_channels, bis, node) {
		...
		store_bis_broadcaster_info(evt, bis);
		bt_conn_set_state(bis->iso, BT_CONN_CONNECTED);
#ifdef CONFIG_LOGSRV_BT_DEBUG
		iso_handle = bis->iso->handle;
#endif
	}
}
...
void hci_le_big_sync_established(struct net_buf *buf)
{
	...
	BT_DBG("BIG[%u] %p sync established,0x%02x \n", big->handle, big, evt->status);
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&big->bis_channels, bis, node) {
		...
		iso_handle = bis->iso->handle;
	#endif
		store_bis_sync_receiver_info(evt, bis);
		bt_conn_set_state(bis->iso, BT_CONN_CONNECTED);
	}
		    ...
		    struct bt_iso_big **out_big)
{
	...
	/* Set connection states before creating sync */
	SYS_SLIST_FOR_EACH_CONTAINER(&big->bis_channels, bis, node) {
		bt_iso_chan_set_state(bis, BT_ISO_CONNECT);
	}
...
int bt_iso_read_rssi(complete_event_cb cb)
{
#ifdef CONFIG_LOGSRV_BT_DEBUG
	struct bt_conn *iso;
	if (!iso_handle) 
	...
	iso = bt_conn_lookup_handle(iso_handle);
	if (iso == NULL) {
		BT_ERR("Unable to find conn for handle %u", iso_handle);
		return -ENOBUFS;
	}

	bt_br_read_rssi(iso, cb);
	bt_conn_unref(iso);
#endif
	return 0;
}

int bt_iso_read_link_info(complete_event_cb cb)
{
#ifdef CONFIG_LOGSRV_BT_DEBUG
	struct bt_conn *iso;
	if (!iso_handle) 
