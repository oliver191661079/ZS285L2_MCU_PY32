...
void bt_sdp_init(void)
{
	...
	BT_INFO("br support capable 0x%x\n",bt_inner_value.br_support_capable);
	if((bt_inner_value.br_support_capable & BT_SUPPORT_A2DP_BIT)
		== BT_SUPPORT_A2DP_BIT){
		bt_register_a2dp_sink_sdp();
	}

	if((bt_inner_value.br_support_capable & BT_SUPPORT_A2DP_SOURCE_BIT)
		== BT_SUPPORT_A2DP_SOURCE_BIT){
		bt_register_a2dp_source_sdp();
	}

	if((bt_inner_value.br_support_capable & BT_SUPPORT_AVRCP_BIT)
		== BT_SUPPORT_AVRCP_BIT){
		bt_register_avrcp_ct_sdp();
		bt_register_avrcp_tg_sdp();
	}

	if((bt_inner_value.br_support_capable & BT_SUPPORT_HFP_HF_BIT)
		== BT_SUPPORT_HFP_HF_BIT){
		bt_register_hfp_hf_sdp();
	}

	if((bt_inner_value.br_support_capable & BT_SUPPORT_HFP_AG_BIT)
		== BT_SUPPORT_HFP_AG_BIT){
		bt_register_hfp_ag_sdp();
	}

#ifdef CONFIG_BT_GATT_OVER_BR
	bt_register_gatt_att_sdp();
#endif
}
...
static struct net_buf *sdp_client_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	if(session->param){
		...
		BT_ERR("session %p chan %p no requst", session, chan);
	}
		    ...
		    const struct bt_sdp_discover_params *params)
{
	...
	if (!session) {
		 ...
		 */
		session = sdp_client_new_session(conn);
		if(!session) {
			return -ENOMEM;
		} else {
			sys_slist_append(&session->reqs, (sys_snode_t *)&params->_node);
			int err = sdp_client_chan_connect(session);
			if (err) {
				(void)memset(session, 0, sizeof(*session));
				BT_ERR("Cannot connect %d", err);
				return -EIO;
...
static int sdp_get_additional_uuid_data(const struct bt_sdp_attr_item *attr,
			  struct bt_sdp_uuid_desc *pd1, struct bt_sdp_uuid_desc *pd2,
			  uint16_t proto_profile1, uint16_t proto_profile2)
{
	/* get start address of attribute value */
	uint8_t *p = attr->val;
	ssize_t slen;
	uint8_t find_protol1 = 0;
	uint8_t find_protol2 = 0;
	uint8_t counter = 0;
	struct bt_sdp_uuid_desc tmp_pd = {0};
	struct bt_sdp_uuid_desc *pd = &tmp_pd;
 ...
 /* start reading stacked UUIDs in analyzed sequences tree */
	while (p - attr->val < attr->len) {
		...
		left -= sizeof(p[0]);
		  ...
		  * If found save it's location and length and return.
		  */
		if (((uuid_type == BT_SDP_UUID16) && (proto_profile1 == BT_UUID_16(&pd->uuid)->val)) ||
			 ((uuid_type == BT_SDP_UUID32) && (proto_profile1 == BT_UUID_32(&pd->uuid)->val))) {
			pd1->params = pd->params;
			pd1->params_len = left;
			if (uuid_type == BT_SDP_UUID16) {
				memcpy(&pd1->uuid16, &pd->uuid16, sizeof(struct bt_uuid_16));
			} else {
				memcpy(&pd1->uuid32, &pd->uuid32, sizeof(struct bt_uuid_32));
			}
			find_protol1 = 1;

		} else if (((uuid_type == BT_SDP_UUID16) && (proto_profile2 == BT_UUID_16(&pd->uuid)->val)) ||
			 ((uuid_type == BT_SDP_UUID32) && (proto_profile2 == BT_UUID_32(&pd->uuid)->val))) {
			pd2->params = pd->params;
			pd2->params_len = left;
			if (uuid_type == BT_SDP_UUID16) {
				memcpy(&pd2->uuid16, &pd->uuid16, sizeof(struct bt_uuid_16));
			} else {
				memcpy(&pd2->uuid32, &pd->uuid32, sizeof(struct bt_uuid_32));
			}
			find_protol2 = 1;
		}

		if (find_protol1 && find_protol2) {
			return 0;
		} else if (counter % 2 == 0 && find_protol2 == 0) {
			find_protol1 = 0;
			find_protol2 = 0;
		}
	...
	BT_DBG("Value 0x%04x 0x%04x not found", proto_profile1, proto_profile2);
	return -ENOENT;
}
...
int bt_sdp_get_additional_proto_param(const struct net_buf *buf, enum bt_sdp_proto proto1, enum bt_sdp_proto proto2,
			 uint16_t *param1, uint16_t *param2)
{
	struct bt_sdp_attr_item attr;
	struct bt_sdp_uuid_desc pd1;
	struct bt_sdp_uuid_desc pd2;
	int res;
	int res2;
	...
	if (res < 0) {
		BT_WARN("Attribute 0x%04x not found, err %d",
			BT_SDP_ATTR_ADD_PROTO_DESC_LIST, res);
		return res;
	}

	res = sdp_get_additional_uuid_data(&attr, &pd1, &pd2, proto1, proto2);
	if (res < 0) {
		BT_WARN("Protocol specifier 0x%04x 0x%04x not found, err %d", proto1, proto2,
			res);
		return res;
	}
	res = sdp_get_param_item(&pd1, param1);
	res2 = sdp_get_param_item(&pd2, param2);

	return (!res || !res2) ? 0 : res;
}


int bt_sdp_get_l2cap_psm(const struct net_buf *buf, uint16_t *psm)
{
	struct bt_sdp_attr_item attr;
	const uint8_t *p;
	int res;

	res = bt_sdp_get_attr(buf, &attr, BT_SDP_ATTR_L2CAP_PSM);
	if (res < 0) {
		BT_WARN("Attribute 0x%04x not found, err %d",BT_SDP_ATTR_L2CAP_PSM, res);
		return res;
	}
	p = attr.val;
	BT_ASSERT(p);
...
int bt_sdp_get_capabilities(const struct net_buf *buf, uint8_t *capabilities)
{
	struct bt_sdp_attr_item attr;
	const uint8_t *p;
	int res;

	res = bt_sdp_get_attr(buf, &attr, BT_SDP_ATTR_SUPPORTED_CAPABILITIES);
	if (res < 0) {
		BT_WARN("Attribute 0x%04x not found, err %d",
			BT_SDP_ATTR_SUPPORTED_CAPABILITIES, res);
		return res;
	}
...
int bt_sdp_get_vendor_id(const struct net_buf *buf,uint16_t *vendor_id)
{
	...
	if (res < 0) {
		BT_WARN("VENDOR_ID 0x%04x not found, err %d",
