...
#if SUPPORT_GATT_OVER_BR
	#define SUB_MAX (CONFIG_BT_MAX_PAIRED + CONFIG_BT_MAX_CONN)
#else
	#define SUB_MAX ((CONFIG_BT_MAX_PAIRED - CONFIG_BT_MAX_BR_PAIRED) + \
...
#else
	#define SC_CFG_MAX ((CONFIG_BT_MAX_PAIRED - CONFIG_BT_MAX_BR_PAIRED) + \
			    (CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN))
#endif
#else
	#define SC_CFG_MAX 0
...
static ssize_t sc_ccc_cfg_write(struct bt_conn *conn, uint8_t conn_type,
...
#else
	#define CF_CFG_MAX ((CONFIG_BT_MAX_PAIRED - CONFIG_BT_MAX_BR_PAIRED) + \
			    (CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN))
#endif
#else
	#define CF_CFG_MAX 0
...
static inline void sc_work_submit(int timeout)
{
#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
			  ...
			  const void *value, uint16_t value_len)
{
	...
	if (value_len != 0U && value == NULL) {
		BT_WARN("value_len of %u provided for NULL value", value_len);
...
static void gatt_ccc_changed(struct bt_conn *conn,
				 const struct bt_gatt_attr *attr, struct _bt_gatt_ccc *ccc)
{
	int i;
	...
	BT_DBG("ccc %p value 0x%04x, 0x%04x", ccc, value, ccc->value);

	if (value != ccc->value) {
		ccc->value = value;
		if (ccc->cfg_changed) {
			ccc->cfg_changed(conn, conn->type, attr, value);
		}
	}
}
			       ...
			       uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (ccc->cfg_write) {
		ssize_t write = ccc->cfg_write(conn, conn->type, attr, value);
...
uint16_t bt_gatt_get_over_br_mtu(struct bt_conn *conn)
{
	return bt_att_get_over_br_mtu(conn);
}
			       ...
			       void *user_data)
{
	...
	if (!value_used) {
		ccc->value = 0U;
		if (ccc->cfg_changed) {
			ccc->cfg_changed(conn, conn->type, attr, ccc->value);
		}
...
void bt_gatt_connected(struct bt_conn *conn)
{
	...
	if (IS_ENABLED(CONFIG_BT_SMP) &&
		...
		if (err) {
			BT_WARN("Failed to set security for bonded peer (%d)", err);
		}
	}
}
...
static int bt_gatt_store_cf(uint8_t id, const bt_addr_le_t *peer)
{
	...
	BT_DBG("Stored CF for %s (%s)", bt_addr_le_str(peer), log_strdup(key));
#endif /* CONFIG_BT_GATT_CACHING */
	return 0;
...
uint16_t bt_gatt_get_attr_by_uuid(uint16_t uuid_val)
{
	struct bt_uuid_16 uuid_16 = {0};
	uuid_16.uuid.type = BT_UUID_TYPE_16;
	uuid_16.val = uuid_val;
	const struct bt_uuid *p_uuid = &uuid_16.uuid;

	struct notify_data found = {0};
	gatt_find_by_uuid(&found, p_uuid);

	return found.handle;
}

int bt_gatt_get_ccc_value(const struct bt_gatt_attr *attr,uint8_t id, const bt_addr_le_t *addr)
{
	char key[BT_PROPERTY_KEY_MAX] = {0};
	char *name;
	ssize_t len;
	if (!attr || !addr) {
		return -EINVAL;
	...
	struct bt_gatt_ccc_cfg *cfg = ccc_find_cfg(ccc, addr, id);
	if(cfg){
		return cfg->value;
		...
		struct ccc_store *ccc_store = bt_malloc(sizeof(struct ccc_store) * CCC_STORE_MAX);
		if (!ccc_store) {
			BT_ERR("no mem");
			return -ENOMEM;
		}
		...
		name = key + strlen(CFG_BT_CCC) + 1;
		len = bt_property_get(key, (char *)ccc_store, sizeof(struct ccc_store) * CCC_STORE_MAX);

		if (len < 0) {
			BT_INFO("Property %s not exist", key);
			bt_free(ccc_store);
			return -EINVAL;
		}

		for (size_t i = 0; i < CCC_STORE_MAX; i++) {
			if(ccc_store[i].handle == bt_gatt_attr_get_handle(attr)){
				BT_INFO("Find %s: handle 0x%04x value 0x%04x",key,
					ccc_store[i].handle, ccc_store[i].value);
				int value = ccc_store[i].value;
				bt_free(ccc_store);
				return value;
			}
		}
