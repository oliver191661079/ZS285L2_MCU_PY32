 ...
 * 3. Controler extend featrues page 2 not ready;
 * 4. Open LE_DEFAULT_STAIC_RANDOM_ADDR need update mac for encryption;
 * 5. Derive the BR/EDR Link Key from the LE LTK, BR/EDR link key need to store;
 * 6. As Central role wait to debug.
*/
...
#if defined(CONFIG_BT_BREDR) && defined(CONFIG_BT_CROSS_TRANSPORT_KEY)
#define LINK_DIST BT_SMP_DIST_LINK_KEY
#define LTK_DIST BT_SMP_DIST_ENC_KEY
#else
#define LINK_DIST 0
#define LTK_DIST  0
#endif

#define RECV_KEYS (LTK_DIST | BT_SMP_DIST_ID_KEY | SIGN_DIST |\
		   LINK_DIST)
#define SEND_KEYS (LTK_DIST | ID_DIST | SIGN_DIST | LINK_DIST)
...
__in_section_unique(BT_MANAGER_BSS) static struct bt_smp_br bt_smp_br_pool[CONFIG_BT_MAX_BR_CONN];
#endif /* CONFIG_BT_BREDR */
...
static uint8_t ctkd_enable = 0;
int bt_ctkd_enable(uint8_t enable)
{
	if (IS_ENABLED(CONFIG_BT_CROSS_TRANSPORT_KEY) &&
		(enable != ctkd_enable)) {
		ctkd_enable = enable;
		BT_INFO("CTKD: %d", ctkd_enable);
	}
	return 0;
}
...
static void sc_derive_link_key(struct bt_smp *smp)
{
	...
	if (atomic_test_bit(smp->flags, SMP_FLAG_CT2)) {
		...
		if (smp_h6(conn->le.keys->ltk.val, tmp1, ilk)) {
			BT_ERR("use smp_h6 error");
			return;
		}
	}

	if (smp_h6(ilk, lebr, link_key.val)) {
		BT_ERR("ilk smp_h6 error");
		return;
	}
...
static void smp_br_derive_ltk(struct bt_smp_br *smp)
{
	...
	BT_INFO("BR linkkey to ltk, LinkKey flag: %d", link_key->flags);
...
#if 0 //todo:1.debug for flags (keys->flags & BT_KEYS_AUTHENTICATED);2.add return?
	if ((keys->flags & BT_KEYS_AUTHENTICATED) && !(link_key->flags & BT_LINK_KEY_AUTHENTICATED)) {
		BT_ERR("The LTK is Authenticated, but linkkey is not, don't derive");
		return;
	}
#endif
...
static uint8_t smp_br_pairing_req(struct bt_smp_br *smp, struct net_buf *buf)
{
	...
	if (!ctkd_enable) {
		rsp->init_key_dist = (req->init_key_dist & BR_RECV_KEYS_SC & ~(BT_SMP_DIST_ENC_KEY));
		rsp->resp_key_dist = (req->resp_key_dist & BR_SEND_KEYS_SC & ~(BT_SMP_DIST_ENC_KEY));
	}
...
static void smp_reset(struct bt_smp *smp)
{
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
		...
		#if 0
		//Workaround for PTS issue: GAP/SEC/SEM/BI-09-C,GAP/SEC/SEM/BI-20-C,
		//GAP/SEC/SEM/BI-21-C. PTS as a slave role to send BT_SMP_CMD_PAIRING_REQ 1st.
		atomic_set_bit(&smp->allowed_cmds, BT_SMP_CMD_PAIRING_REQ);
		#endif
...
static void smp_pairing_complete(struct bt_smp *smp, uint8_t status)
{
	...
	if (conn->le.keys == NULL) {
		 ...
		 */
		BT_WARN("The in-progress pairing has been deleted!");
...
int bt_sc_enable(uint8_t enable)
{
	...
	if (BT_FEAT_SC(bt_dev.features) && IS_ENABLED(CONFIG_BT_BR_SC_HOST_SUPP)) {
		...
		sc_cp->sc_support = (enable > 0)? 0x01 : 0x00;
		...
		BT_INFO("SC: %ld, %d",
			...
			IS_ENABLED(CONFIG_BT_BR_SC_HOST_SUPP));
	}
	return 0;
}
...
static uint8_t smp_pairing_req(struct bt_smp *smp, struct net_buf *buf)
{
	...
	if (!conn->le.keys) {
		...
		memset(conn->le.keys,0,sizeof(struct bt_keys));
	}
	...
	if (!ctkd_enable) {
		rsp->init_key_dist = (req->init_key_dist & RECV_KEYS & ~(LINK_DIST));
		rsp->resp_key_dist = (req->resp_key_dist & SEND_KEYS & ~(LINK_DIST));
	}
	...
	if(!notify_remote_linkkey_miss(conn)){
		return BT_SMP_ERR_UNSPECIFIED;
	}
...
static uint8_t compute_and_check_and_send_slave_dhcheck(struct bt_smp *smp)
{
	...
	if (memcmp(smp->e, re, 16)) {
		BT_ERR("BT_SMP_ERR_DHKEY_CHECK_FAILED");
