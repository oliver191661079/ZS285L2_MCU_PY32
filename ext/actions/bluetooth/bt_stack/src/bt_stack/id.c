...
void bt_id_add(struct bt_keys *keys)
{
	...
	err = bt_le_whitelist_add(&keys->addr);
	if (err) {
		BT_ERR("add whitelist error: %d", err);
	}
#endif
...
#ifdef CONFIG_BT_WHITELIST
	err = bt_le_whitelist_rem(&keys->addr);
	if (err) {
		BT_ERR("remove from whitelist error: %d", err);
	}
#endif
...
int bt_setup_public_id_addr(void)
{
	...
	return id_create(BT_ID_PUBLIC, &addr, irk);
}
	...
	if (options & BT_LE_ADV_OPT_CONNECTABLE) {
		...
		if (IS_ENABLED(CONFIG_BT_PRIVACY) &&
			...
			*own_addr_type = id_addr->type;
			...
			} else if ((options & BT_LE_ADV_OPT_ADDR_RPA) && (options & BT_LE_ADV_OPT_USE_IDENTITY)){
				if (id_addr->type == BT_ADDR_LE_RANDOM){
					*own_addr_type = BT_HCI_OWN_ADDR_RPA_OR_RANDOM;
...
int bt_id_init(void)
{
	...
	if (!IS_ENABLED(CONFIG_BT_PROPERTY) && bt_dev.id_count < CONFIG_BT_ID_MAX) {
...
int bt_le_set_rpa_timeout(uint16_t timeout)
{
	...
	cp = net_buf_add(buf, sizeof(*cp));
	cp->rpa_timeout = timeout;
	ret = bt_hci_cmd_send_sync(BT_HCI_OP_LE_SET_RPA_TIMEOUT, buf, NULL);
