...
#if defined(CONFIG_BT_LETWS)
#define	LETWS_MAX_CONN		1
#else
#define	LETWS_MAX_CONN		0
#endif
#define LE_MAX_CONN	3//(CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN - LETWS_MAX_CONN)
...
#else
K_MEM_SLAB_DEFINE(req_slab, sizeof(struct bt_att_req),
		  CONFIG_BT_L2CAP_TX_BUF_COUNT + 20, __alignof__(struct bt_att_req));

NET_BUF_POOL_DEFINE(att_tx_pool, 20, 30, 4, NULL);
#endif
...
K_MEM_SLAB_DEFINE(att_slab, sizeof(struct bt_att),
		  (CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN) + (CONFIG_BT_MAX_BR_CONN * SUPPORT_GATT_OVER_BR),
		  __alignof__(struct bt_att));
K_MEM_SLAB_DEFINE(chan_slab, sizeof(struct bt_att_chan),
		   LE_MAX_CONN * ATT_CHAN_MAX + (CONFIG_BT_MAX_BR_CONN * SUPPORT_GATT_OVER_BR) + LETWS_MAX_CONN,
...
static uint16_t bt_att_mtu = BT_ATT_MTU;

uint16_t bt_att_get_cfg_mtu(void)
{
	return bt_att_mtu;
}

void bt_att_set_cfg_mtu(uint16_t att_mtu)
{
	bt_att_mtu = att_mtu;
}
		     ...
		     bt_att_chan_sent_t cb)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR) &&
		...
		(chan->att->conn->type == BT_CONN_TYPE_BR)) {
		err = bt_l2cap_send_cb(chan->att->conn, chan->br_chan.tx.cid,
...
static int chan_req_send(struct bt_att_chan *chan, struct bt_att_req *req)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR) &&
		(bt_internal_support_gatt_over_br() == 1) &&
		(chan->att->conn->type == BT_CONN_TYPE_BR)) {
		tx_mtu = chan->br_chan.tx.mtu;
	}else {
		tx_mtu = chan->chan.tx.mtu;
	}

	if (tx_mtu < net_buf_frags_len(req->buf)) {
		return -EMSGSIZE;
	}
...
static void bt_att_sent(struct bt_l2cap_chan *ch)
{
	...
	/*
	 * bt_att_chan is managed by slab, so the fisrt member (att)
	 * will be used internally by slab after free.
	 * If 1 EATT channel is disconnected while the connection is
	 * not, bt_att_sent() may be called after att_chan_detach(),
	 * which means the channel is diconnected but "chan->att" is
	 * not NULL.
	 * So we should check the channel with ATT_CONNECTED first.
	 */
	if (!atomic_test_bit(chan->flags, ATT_CONNECTED)) {
		BT_DBG("disconnected %p", chan);
		return;
	}
				       ...
				       size_t len)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR) &&
		...
		(chan->att->conn->type == BT_CONN_TYPE_BR)) {
		tx_mtu = chan->br_chan.tx.mtu;
		BT_DBG("chan %p, rx.cid: 0x%x, tx.cid: 0x%x\r\n",
				&chan->br_chan, chan->br_chan.rx.cid, chan->br_chan.tx.cid);
	}else {
		tx_mtu = chan->chan.tx.mtu;
	}

	if (len + sizeof(op) > tx_mtu) {
		BT_WARN("ATT MTU exceeded, max %u, wanted %zu",
			tx_mtu, len + sizeof(op));
		return NULL;
	}

	if ((att_op_get_type(op) != ATT_RESPONSE) && (len + sizeof(op) <= 20)) {
		pool = &att_tx_pool;
	}
	...
	case ATT_CONFIRMATION:
		...
		buf = bt_l2cap_create_pdu_timeout(pool, 0, BT_ATT_TIMEOUT);
		break;
	default:
			    ...
			    void *user_data)
{
	...
	if (tx_mtu - net_buf_frags_len(data->buf) <
	    sizeof(*data->group)) {
		return BT_GATT_ITER_STOP;
	}
	...
	len = MIN(tx_mtu - net_buf_frags_len(data->buf),
			     ...
			     attr_read_cb cb, void *user_data)
{
	...
	if (tx_mtu <= net_buf_frags_len(buf)) {
		return 0;
	}
	 ...
	 */
	do {
		len = MIN(tx_mtu - net_buf_frags_len(buf),
			  net_buf_tailroom(frag));
		if (!len) {
		...
		total += read;
		offset += read;
	} while (tx_mtu > net_buf_frags_len(buf) && read == len);
			    ...
			    void *user_data)
{
	...
	return tx_mtu - net_buf_frags_len(data->buf) >
			     ...
			     void *user_data)
{
	...
	/* Stop if there is no space left */
	if (data->rsp->len &&
	    tx_mtu - data->buf->len < data->rsp->len) {
		return BT_GATT_ITER_STOP;
	}
...
static uint8_t att_error_rsp(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	if (!att_change_security(chan->att->conn, err)) {
...
{
	struct bt_att_chan *att_chan = NULL;
	struct bt_att_hdr *hdr;
    ...
    if (IS_ENABLED(CONFIG_BT_BREDR) &&
		...
		//disguised as LE connection
		chan->conn->le.dst.type = 0x00;
		bt_addr_copy(&chan->conn->le.dst.a, &chan->conn->br.dst);
		bt_addr_le_copy(&chan->conn->le.resp_addr, &chan->conn->le.dst);
		bt_addr_le_copy(&chan->conn->le.init_addr, &chan->conn->le.dst);
...
/*
 * If ATT channel is disconnected but EATT channel(s) not,
 * find any one valid bt_l2cap_chan and get the related bt_att.
 */
static struct bt_att *find_att_by_eatt_chan(struct bt_conn *conn)
{
	struct bt_att_chan *att_chan;
	struct bt_l2cap_chan *chan;

	SYS_SLIST_FOR_EACH_CONTAINER(&conn->channels, chan, node) {
		if(chan->psm == BT_EATT_PSM){
			if (IS_ENABLED(CONFIG_BT_BREDR) &&
				(bt_internal_support_gatt_over_br() == 1) &&
				(conn->type == BT_CONN_TYPE_BR)) {
				att_chan = ATT_BR_CHAN(chan);
...
static struct bt_att *att_get(struct bt_conn *conn)
{
	...
	if (!atomic_test_bit(att_chan->flags, ATT_CONNECTED)) {
		BT_WARN("ATT channel not connected");
	}
#endif

	if (!att_chan->att) {
		return find_att_by_eatt_chan(conn);
	}
...
static void att_timeout(struct k_work *work)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR) &&
		(bt_internal_support_gatt_over_br() == 1) &&
		(chan->att->conn->type == BT_CONN_TYPE_BR)) {
		bt_att_disconnected(&chan->br_chan.chan);
	}else {
...
static void bt_att_connected(struct bt_l2cap_chan *chan)
{
    ...
    if (IS_ENABLED(CONFIG_BT_BREDR) &&
		...
		Z_STRUCT_SECTION_FOREACH(bt_gatt_over_br_user_info, info) {
			info->user_cb(chan->conn, true);
			break;
		}
	}else {
		att_chan = ATT_CHAN(chan);
	...
	if (!att_chan->att) {
		BT_WARN("Ignore disconnected ATT chan");
		return;
	}
	...
	if ((ch) &&
		(!atomic_test_bit(att_chan->flags, ATT_ENHANCED))) {
...
static void bt_att_disconnected(struct bt_l2cap_chan *chan)
{
	...
	/* Ignore EATT channels that are not connected */
	if (IS_ENABLED(CONFIG_BT_EATT) &&
	    atomic_test_bit(att_chan->flags, ATT_ENHANCED)) {
		if (!sys_slist_find(&att->chans, &att_chan->node)) {
			BT_INFO("Ignore EATT");
			return;
	    }
	}
	...
	atomic_clear_bit(att_chan->flags, ATT_CONNECTED);
...
static int bt_att_accept(struct bt_conn *conn, struct bt_l2cap_chan **ch)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR) &&
		(bt_internal_support_gatt_over_br() == 1) &&
		(conn->type == BT_CONN_TYPE_BR)) {
		chan->br_chan.rx.mtu = BT_ATT_OVER_BR_MTU;
		chan->br_chan.tx.mtu = BT_ATT_OVER_BR_MTU;
...
static struct bt_l2cap_server gatt_over_br_l2cap = {
	.psm = 0x1f,
...
/* init function */
int bt_gatt_over_br_init(void)
{
	int err;

	BT_DBG("");

	/* Register GATT_OVER_BR PSM with L2CAP */
	err = bt_l2cap_br_server_register(&gatt_over_br_l2cap);
	if (err < 0) {
		BT_ERR("GATT_OVER_BR L2CAP Registration failed %d", err);
	}
...
uint16_t bt_att_get_over_br_mtu(struct bt_conn *conn)
{
	struct bt_att_chan *chan, *tmp;
	struct bt_att *att;
	uint16_t mtu = 0;
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&att->chans, chan, tmp, node) {
		if ((att->conn->type == BT_CONN_TYPE_BR) && (chan->br_chan.tx.mtu > mtu)) {
...
struct bt_att_req *bt_att_req_alloc(int timeout)
{
...
void bt_att_sync_get_mtu(struct bt_conn *conn, uint16_t *rx_mtu, uint16_t *tx_mtu)
{
	...
	att_chan = ATT_CHAN(chan);
	*rx_mtu = att_chan->chan.rx.mtu;
	*tx_mtu = att_chan->chan.tx.mtu;
}
...
void bt_att_sync_set_mtu(struct bt_conn *conn, uint16_t rx_mtu, uint16_t tx_mtu)
{
	...
	att_chan = ATT_CHAN(chan);
	att_chan->chan.rx.mtu = rx_mtu;
	att_chan->chan.tx.mtu = tx_mtu;
}
