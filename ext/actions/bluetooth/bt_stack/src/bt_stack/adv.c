...
static bool valid_adv_ext_param(const struct bt_le_adv_param *param)
{
	if (IS_ENABLED(CONFIG_BT_EXT_ADV) &&
		...
		if (param->peer &&
		    (param->options & BT_LE_ADV_OPT_EXT_ADV) &&
		    !(param->options & (BT_LE_ADV_OPT_DIR_MODE_LOW_DUTY | BT_LE_ADV_OPT_ADDR_RPA))) {
		...
		if ((param->options & BT_LE_ADV_OPT_ADDR_RPA) &&
				...
				const struct bt_data *ad, size_t ad_len)
{
	...
	while (!ad_stream_is_empty(&stream)) {
		...
		if (is_first_iteration && ad_stream_is_empty(&stream)) {
			set_data->op = (BT_HCI_LE_EXT_ADV_OP_COMPLETE_DATA | ack_flag);
		} else if (is_first_iteration) {
			set_data->op = (BT_HCI_LE_EXT_ADV_OP_FIRST_FRAG | ack_flag);
		} else if (ad_stream_is_empty(&stream)) {
			set_data->op = (BT_HCI_LE_EXT_ADV_OP_LAST_FRAG | ack_flag);
		} else {
			set_data->op = (BT_HCI_LE_EXT_ADV_OP_INTERM_FRAG | ack_flag);
		}
...
static int le_adv_start_add_conn(struct bt_le_ext_adv *adv,
				 struct bt_conn **out_conn)
{
	...
	if(adv->conn){
		BT_ERR("add conn failed %p %p\n",adv,adv->conn);
		return -EINVAL;
	}

	if (!bt_addr_le_cmp(&adv->target_addr, BT_ADDR_LE_ANY) || (adv->options & BT_LE_ADV_OPT_ADDR_RPA)) {
		...
		*out_conn = conn;
		adv->conn = conn;
		if(adv != bt_le_adv_lookup_legacy()){
			BT_INFO("add conn %p %p %p\n",adv,bt_le_adv_lookup_legacy(),conn);
		}
		return 0;
	}
...
static void le_adv_stop_free_conn(struct bt_le_ext_adv *adv, uint8_t status)
{
	struct bt_conn *conn;
#if 0
	if (!bt_addr_le_cmp(&adv->target_addr, BT_ADDR_LE_ANY) || (adv->options & BT_LE_ADV_OPT_ADDR_RPA)) {
	...
	if (conn) {
		if(adv != bt_le_adv_lookup_legacy()){
			BT_INFO("free conn %p %p %p\n",adv,bt_le_adv_lookup_legacy(),conn);
		}
        bt_conn_le_adv_update(conn);
		conn->err = status;
				...
				bool  has_scan_data)
{
	...
	bool dir_adv = (param->options & (BT_LE_ADV_OPT_DIR_ADDR_RPA | BT_LE_ADV_OPT_DIR_MODE_LOW_DUTY)) ? true : false, scannable;
	...
	if (param->peer &&
		(param->options & (BT_LE_ADV_OPT_DIR_MODE_LOW_DUTY|BT_LE_ADV_OPT_DIR_ADDR_RPA|BT_LE_ADV_OPT_ADDR_RPA))){
			...
			const struct bt_data *sd, size_t sd_len)
{
	...
	bool dir_adv = (param->options & (BT_LE_ADV_OPT_DIR_ADDR_RPA | BT_LE_ADV_OPT_DIR_MODE_LOW_DUTY)) ? true : false;
	struct bt_conn *conn = NULL;
	int err;
...
{
	return k_delayed_work_cancel(&adv->lim_adv_timeout_work);
}
		    ...
		    const struct bt_data *sd, size_t sd_len)
{
	...
	if (ad_is_limited(ad, ad_len)) {
		k_delayed_work_init(&adv->lim_adv_timeout_work, adv_timeout);
		k_delayed_work_submit(&adv->lim_adv_timeout_work,
...
static void adv_timeout(struct k_work *work)
{
	...
	dwork = CONTAINER_OF(work, struct k_delayed_work, work);
...
void bt_hci_le_per_adv_subevent_data_request(struct net_buf *buf)
{
	...
	if (!adv) {
		BT_ERR("Unknown advertising handle %d", evt->adv_handle);
...
void bt_hci_le_per_adv_response_report(struct net_buf *buf)
{
	...
	if (buf->len < sizeof(struct bt_hci_evt_le_per_adv_response_report)) {
		BT_ERR("Invalid response report");
	...
	for (uint8_t i = 0; i < evt->num_responses; i++) {
		...
		if (response->data_status == BT_HCI_LE_ADV_EVT_TYPE_DATA_STATUS_PARTIAL) {
			BT_WARN("Incomplete response report received, discarding");
			...
			BT_ERR("Invalid data status %d", response->data_status);
			(void)net_buf_pull_mem(buf, response->data_length);
		}
	}
}
	...
	for (int i = 0; i < ARRAY_SIZE(bt_dev.cached_conn_complete); i++) {
		if (bt_dev.cached_conn_complete[i].valid &&
			...
			/*if (was_adv_enabled)*/
			{
				 ...
				 * discard the event.
				 */
				bt_dev.cached_conn_complete[i].adv = adv;
