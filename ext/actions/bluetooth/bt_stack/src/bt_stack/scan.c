...
int bt_le_scan_update(bool fast_scan)
{
	...
	if (get_pending_per_adv_sync()) {
		//To prevent scan from being opened again after 
		//ble_scan_stop is executed during the PA sync process
		//return start_passive_scan(fast_scan);
	}
#endif
...
static void bt_hci_le_per_adv_report_common(struct net_buf *buf)
{
	...
	if (!per_adv_sync->report_truncated) {
		...
		if (net_buf_simple_tailroom(&per_adv_sync->reassembly) < evt->length) {
			...
			BT_WRN("Buffer is too small to reassemble the report. "
		...
		if (evt->data_status == BT_HCI_LE_ADV_EVT_TYPE_DATA_STATUS_COMPLETE) {
			...
			BT_DBG("Received incomplete advertising data. "
...
#endif /*#if defined(CONFIG_BT_PER_ADV_SYNC_RSP)*/

void bt_hci_le_per_adv_report(struct net_buf *buf)
{
#if 0
	if (IS_ENABLED(CONFIG_BT_PER_ADV_SYNC_RSP)) {
		BT_ERR("The controller shall raise the latest unmasked version of the event");
	...
	if (!pending_per_adv_sync ||
	    (!atomic_test_bit(pending_per_adv_sync->flags,
	     ...
	     ((pending_per_adv_sync->sid != evt->sid) ||
	      bt_addr_le_cmp(&pending_per_adv_sync->addr, &id_addr)))) {
	...
	if (evt->status) {
        ...
        if (evt->status == BT_HCI_ERR_CONN_FAIL_TO_ESTAB) {
	       ...
	       /* Terminate the PA sync and notify app */
			...
			if(tmp_term_info.conn){
				bt_conn_unref(tmp_term_info.conn);
			}
		}
		return;
	}
