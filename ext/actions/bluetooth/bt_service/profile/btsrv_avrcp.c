...
#define AVRCP_CTRL_VOL_SYNC_RSSI_THRESHOLD		-80
...
static void _btsrv_avrcp_ctrl_update_support_feature(struct bt_conn *conn, uint16_t feature_mask, uint16_t l2cap_psm)
{
	uint32_t data[3] = {0};
	data[0] = (uint32_t)conn;
	data[1] = feature_mask;
	data[2] = l2cap_psm;
	btsrv_event_notify_malloc(MSG_BTSRV_AVRCP, MSG_BTSRV_AVRCP_UPDATE_SUPPORT_FEATURE, (uint8_t *)data, sizeof(data), 0);
}
static const struct bt_avrcp_app_cb btsrv_avrcp_ctrl_cb = {
    ...
    .update_support_feature = _btsrv_avrcp_ctrl_update_support_feature,
};
...
static void connected_dev_cb_sync_volume(struct bt_conn *base_conn, uint8_t tws_dev, void *cb_param)
{
    ...
    rssi = btsrv_rdm_get_dev_rssi(base_conn);
    if(rssi < AVRCP_CTRL_VOL_SYNC_RSSI_THRESHOLD){
        SYS_LOG_ERR("rssi low! %d",rssi);
...
static void btsrv_avrcp_sync_vol(void *param)
{
	...
	if (!thread_timer_is_running(&sync_volume_timer)) {
		thread_timer_start(&sync_volume_timer, 0, AVRCP_SYNC_TIMER_INTERVAL);
	}
}
...
static int btsrv_avrcp_update_support_feature(struct bt_conn* conn, uint16_t feature_mask, uint16_t covrt_art_psm)
{
	btsrv_avrcp_evt_update_support_feature feature;
	uint16_t acl_hdl = hostif_bt_conn_get_handle(conn);
	if (avrcp_user_callback && avrcp_user_callback->event_cb) {
		feature.feature_mask = feature_mask;
		feature.covrt_art_psm = covrt_art_psm;
		avrcp_user_callback->event_cb(acl_hdl, BTSRV_AVRCP_UPDATE_SUPPORT_FEATURE, &feature);
	}
	return 0;
}

static int btsrv_avrcp_get_support_feature(uint16_t hdl)
{
	struct bt_conn *avrcp_conn;
	...
	return hostif_bt_avrcp_ct_get_support_feature(avrcp_conn);
}
...
int btsrv_avrcp_process(struct app_msg *msg)
{
	...
	case MSG_BTSRV_AVRCP_NOTIFY_VOLUME_CHANGE:
		...
		break;
	case MSG_BTSRV_AVRCP_UPDATE_SUPPORT_FEATURE:
		SYS_LOG_INF("AVRCP_UPDATE_SUPPORT_FEATURE\n");
		uint32_t* data = msg->ptr;
		btsrv_avrcp_update_support_feature((struct bt_conn*)data[0], (uint16_t)data[1], (uint16_t)data[2]);
		break;
	case MSG_BTSRV_AVRCP_GET_SUPPORT_FEATURE:
		btsrv_avrcp_get_support_feature(msg->value);
		break;
	default:
		break;
	}
