...
void* hal_ain_channel_open(audio_in_init_param_t *init_param)
{
...
#if defined(CONFIG_AUDIO_IN_I2SRX0_SUPPORT) || defined(CONFIG_AUDIO_IN_I2SRX1_SUPPORT)
	i2srx_setting_t i2srx_setting = {0};
#endif
#ifdef CONFIG_AUDIO_IN_SPDIFRX_SUPPORT
	spdifrx_setting_t spdifrx_setting = {0};
#endif
	SYS_LOG_INF("type %d sr %d width %d", init_param->channel_type, 
		init_param->sample_rate, init_param->data_width);
	...
	ain_param.channel_width = init_param->data_width == 32 ? CHANNEL_WIDTH_24BITS : CHANNEL_WIDTH_16BITS;
	...
	switch (init_param->channel_type) {
		case AUDIO_CHANNEL_ADC:
		{
			...
			adc_setting.gain.ch_gain[0] = init_param->input_gain;
			adc_setting.gain.ch_gain[1] = init_param->adc_gain;
			SYS_LOG_INF("adc gain:%d %d\n", adc_setting.gain.ch_gain[0], adc_setting.gain.ch_gain[1]);
...
int hal_ain_channel_set_interleave_mode(void* ain_channel_handle, unsigned int interleave_mode)
{
	...
	return audio_in_control(audio_in->ain_dev, ain_channel_handle, AIN_CMD_SET_INTERLEAVED_MODE, &interleave_mode);
