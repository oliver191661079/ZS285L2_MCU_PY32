...
typedef struct iterator_ops {
	...
	/* (optional) scan plist info in few times*/
	char * (*scan_disk)(struct iterator *iter, void *param);

	/* (optional) scan plist info in few times*/
	int (*has_found_cursor)(struct iterator *iter);

	/* (optional) need plist file start to scan disk update list*/
	int (*need_update_list)(struct iterator *iter);
