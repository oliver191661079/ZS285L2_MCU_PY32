...
const void *iterator_next(struct iterator *iter, bool force_switch, uint16_t *track_no)
{
	...
	if (iter && iter->ops && iter->ops->next)
...
const void *iterator_next_folder(struct iterator *iter, uint16_t *track_no)
{
	...
	if (iter && iter->ops && iter->ops->next_folder)
...
const void *iterator_prev_folder(struct iterator *iter, uint16_t *track_no)
{
	...
	if (iter && iter->ops && iter->ops->prev_folder)
...
const void *iterator_set_track_no(struct iterator *iter, uint16_t track_no)
{
	...
	if (iter && iter->ops && iter->ops->set_track_no)
...
{
	if (iter && iter->ops && iter->ops->get_plist_info)
		return iter->ops->get_plist_info(iter, param);

	return -ENOSYS;
}

char* iterator_scan_disk(struct iterator *iter, void *param)
{
	if (iter && iter->ops && iter->ops->scan_disk)
		return iter->ops->scan_disk(iter, param);

	return NULL;
}

int iterator_has_found_cursor(struct iterator *iter)
{
	if (iter && iter->ops && iter->ops->has_found_cursor)
		return iter->ops->has_found_cursor(iter);

	return -1;
}

int iterator_need_update_list(struct iterator *iter)
{
	if (iter && iter->ops && iter->ops->need_update_list)
		return iter->ops->need_update_list(iter);
