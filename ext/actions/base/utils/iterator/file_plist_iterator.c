...
#define CONFIG_ITERATER_RAM_STATIC
...
{
#ifndef CONFIG_ITERATER_RAM_STATIC
...
static int file_dirname_get(struct play_list_t *plist, struct iterator *iter)
{
	...
	snprintf(data->full_path, FULL_PATH_LEN, "%s%s/%s%u/%lu/%u/%s", OPEN_MODE, plist->topdir, CLUSTER,
		plist->folder_info[plist->folder_seq_num]->cur_cluster, dp->blk_ofs, entry->size, file_format);
...
#if CONFIG_SUPPORT_FILE_FULL_NAME
static int updata_far_cluster_blkofs(struct play_list_t *plist, fs_dir_t *zdp, uint8_t folder_index, const char *path)
{
	DIR* dp = &zdp->dp;
	int res = 0;
...
static int file_iterator_playlist_set(struct play_list_t *plist, fs_dir_t *zdp, uint8_t folder_index, uint8_t layer)
{
	DIR* dp = &zdp->dp;
...
#ifdef CONFIG_ITERATER_RAM_STATIC
__in_section_unique(ITERATER_RAM) static char iterater_ram[0xa00];
#endif
...
static int file_iterator_init(struct iterator *iter, const void *param)
{
	...
	int i = 0;
#ifdef CONFIG_ITERATER_RAM_STATIC
	uint32_t mem_ptr = (uint32_t)iterater_ram;
	memset(iterater_ram, 0, sizeof(iterater_ram));
	SYS_LOG_INF("mem_ptr start:0x%x\n", mem_ptr);
#endif
...
#else
	data->dirent = (void *)mem_ptr;
	mem_ptr += sizeof(*data->dirent);
#endif
	if (!data->dirent)
		goto err_out;
...
#else
		data->dirs[i] =  (void *)mem_ptr;
		mem_ptr += sizeof(fs_dir_t);
#endif
		if (!data->dirs[i])
			goto err_out;
	}
...
#else
	data->dname_len = (void *)mem_ptr;
	mem_ptr += sizeof(*data->dname_len) * data->max_level;
#endif
	if (!data->dname_len)
		goto err_out;
#ifndef CONFIG_ITERATER_RAM_STATIC
...
#else
	data->full_path = (void *)mem_ptr;
	mem_ptr += FULL_PATH_LEN;
#endif
	if (!data->full_path)
		goto err_out;
...
#else
		play_list = (void *)mem_ptr;
		mem_ptr += sizeof(*play_list);
#endif
		if (!play_list)
			goto err_out;
...
#else
			play_list->folder_info[i] = (void *)mem_ptr;
			mem_ptr += sizeof(struct folder_info_t);
#endif
			if (!play_list->folder_info[i])
				goto err_out;
		}
	...
	iter->data = data;
#ifdef CONFIG_ITERATER_RAM_STATIC
	SYS_LOG_INF("mem_ptr end:0x%x size:0x%x\n", mem_ptr, mem_ptr - (uint32_t)iterater_ram);
	if (mem_ptr - (uint32_t)iterater_ram > sizeof(iterater_ram)) {
		SYS_LOG_ERR();
		goto err_out;
	}
#endif
