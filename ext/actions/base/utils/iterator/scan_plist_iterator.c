...
static int file_iterator_playlist_set(struct play_list_t *plist, fs_dir_t *zdp, uint8_t folder_index, uint8_t layer)
{
	...
	SYS_LOG_INF("%s folder_index=%d,cur_cluster= %lu,layer=%d\n",__func__, folder_index, dp->clust, layer);
	return 0;
}
...
exit:
 ...
 inline void delay(volatile unsigned int count)
{
	while(count-- != 0);
}


static char scaning_temp_path[FULL_PATH_LEN];
static char *file_iterator_scan_disk(struct iterator *iter, void *param)
{
	...
	do {
		if((((k_cycle_get_32() - begin)/24) > 20*1000))
		{
			if(plist->scaning_full_path)
			{
		...
		if (data->dirent->type == FS_DIR_ENTRY_FILE && plist->is_file_type) {
			...
			if (cursor && cursor->path &&
				...
				plist->file_seq_num = plist->sum_file_count;
				plist->folder_seq_num = plist->sum_folder_count;
				SYS_LOG_INF("found_cur cur file_seq_num=%d,dir_file_seq_num=%d,folder_seq_num=%d\n",
			...
			if(!cursor_cluster && !cursor_blk_ofs && !cursor_file_size && !plist->found_cursor)
			{
				...
				SYS_LOG_INF("no_break_found_cur file_seq_num=%d,dir_file_seq_num=%d,folder_seq_num=%d\n",
					plist->file_seq_num, plist->dir_file_seq_num, plist->folder_seq_num);
...
static int file_iterator_update_playlist(struct play_list_t *plist, struct iterator *iter, const void *param)
{
	...
	uint32_t begin = k_cycle_get_32();
#endif
	//file_iterator_scan_disk(iter, (void*)param);
#if CONFIG_SYS_LOG_DEFAULT_LEVEL >= 3
