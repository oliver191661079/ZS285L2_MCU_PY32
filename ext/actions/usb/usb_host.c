...
#define SYS_LOG_DOMAIN "usb_host"
...
static inline int hub_port_init(int port1)
{
	...
	k_sleep(K_MSEC(1));
	SYS_LOG_INF("udisk speed: %d", usbh_dev.speed);
...
#else
	for(enum_cnt=0; enum_cnt<MAX_ENUM_CNT; enum_cnt++) {
		ret = usbh_get_device_descriptor(64);
		SYS_LOG_INF("enum_cnt:%d", enum_cnt);
		k_sleep(K_MSEC(1));
		if(ret==sizeof(struct usb_device_descriptor))
			break;
	}
	if(ret<8)
		return -EIO;
