...
#ifdef CONFIG_GPIO_ACTS
static void i2c_gpio_acts_line(u32_t pin, int high)
{
	if (high) {
		sys_write32(I2C_GPIO_PIN_RELEASE, GPIO_CTL(pin));
	} else {
		sys_write32(I2C_GPIO_PIN_DRIVE_LOW, GPIO_CTL(pin));
		sys_write32(GPIO_BIT(pin), GPIO_REG_BRR(GPIO_REG_BASE, pin));
	}
}

static int i2c_gpio_acts_read(u32_t pin)
{
	return !!(sys_read32(GPIO_REG_IDAT(GPIO_REG_BASE, pin)) & GPIO_BIT(pin));
}

static void i2c_gpio_acts_setup_pins(struct i2c_gpio_context *context)
{
	i2c_gpio_acts_line(context->scl_pin, 1);
	i2c_gpio_acts_line(context->sda_pin, 1);
}
#endif
...
#ifdef CONFIG_GPIO_ACTS
	i2c_gpio_acts_line(context->scl_pin, state);
#else
	if (state)
...
#ifdef CONFIG_GPIO_ACTS
	i2c_gpio_acts_line(context->sda_pin, state);
#else
	if (state)
...
static int i2c_gpio_get_sda(void *io_context)
{
	struct i2c_gpio_context *context = io_context;
#ifdef CONFIG_GPIO_ACTS
	return i2c_gpio_acts_read(context->sda_pin);
#else
...
static int i2c_gpio_get_scl(void *io_context)
{
	struct i2c_gpio_context *context = io_context;
#ifdef CONFIG_GPIO_ACTS
	return i2c_gpio_acts_read(context->scl_pin);
#else
	u32_t state = 1; /* Default high as that would be a NACK */
...
#ifdef CONFIG_GPIO_ACTS
	i2c_gpio_acts_setup_pins(context);
#else
