...
#define ACT_LOG_MODEL_ID ALF_MODEL_NVRAM_CONFIG
	...
	u32_t dest_offset, int len)
{
	...
	while (len > 0) {
		...
		err = region_write(region, dest_offset, buf, write_size);
		if (err) {
			return err;
		}
		    ...
		    u32_t old_seg_offset, int copy_size, int check_crc)
{
	...
	while (item_offs < (old_seg_offset + copy_size)) {
		...
		SYS_LOG_INF("item_offs: status 0x%x", status);
		...
		if (status == ITEM_STATUS_VALID) {
			...
			SYS_LOG_INF("valid item: copy from 0x%x to 0x%x, len 0x%x",
	...
	SYS_LOG_INF("new_seg write offset 0x%x", region->seg_write_offset);
...
static int region_purge_seg(struct region_info *region, int check_crc)
{
	...
	SYS_LOG_INF("purge seg offset 0x%x\n", region->seg_offset);
...
static void region_traverse_data(struct region_info *region, u32_t offset, s32_t size, uint8_t *data, uint32_t data_size, void * priv_data, void (*traverse_cb)(uint8_t *data, uint32_t size, void *priv_data))
{
	s32_t pos, read_size;
	...
	while (size > 0) {
		...
		region_read(region, offset + pos, data, read_size);

		traverse_cb(data, read_size, priv_data);
...
static void region_traverse(struct region_info *region, uint8_t *data_buf, uint32_t data_len, void *priv_data, void (*name_cb)(uint8_t *buf, uint32_t size, void *priv_data), void (*data_cb)(uint8_t *buf, uint32_t size, void *priv_data))
{
	struct nvram_item item;
	u32_t offs, item_offs;
	int i;
	...
	while (offs < region->seg_size) {
		...
		/* read config name */
		region_read(region, item_offs + sizeof(struct nvram_item),
			data_buf, item.name_size);

		if(name_cb){
			name_cb(data_buf, item.name_size, priv_data);
		}

		region_traverse_data(region, item_offs + sizeof(struct nvram_item) + item.name_size,
			item.data_size, data_buf, data_len, priv_data, data_cb);
next:
		offs += item_get_aligned_size(&item);
...
static bool is_valid_merge_region_seg_header(struct region_seg_header *hdr)
{
	u32_t crc;

	/* validate magic number and state*/
	if (hdr->magic != NVRAM_REGION_SEG_MAGIC ||
	    hdr->state != NVRAM_REGION_SEG_STATE_OBSOLETE)
		return false;
	...
	crc = calc_crc8((u8_t *)hdr + NVRAM_REGION_SEG_HEADER_CRC_OFFSET,
		sizeof(struct region_seg_header) - NVRAM_REGION_SEG_HEADER_CRC_OFFSET, 0);
	if (hdr->crc != crc) {
		printk("invalid header crc 0x%x != hdr->crc 0x%x\n", crc, hdr->crc);
		return false;
	}
...
static int region_seg_scan(struct region_info *region, int force_merge)
{
	struct nvram_item item;
	...
	while (offs < region->seg_size) {
		...
		SYS_LOG_INF("item_offs 0x%x: status 0x%x", item_offs, status);
		...
		if (status == ITEM_STATUS_VALID) {
		    ...
		    if(item_get_item_name(region, item_offs, &item, item_name, sizeof(item_name)) == 0){
                ...
                if(old_item_offs > 0 && (old_item_offs != item_offs)){
                    SYS_LOG_WRN("same item %s found new:0x%x old 0x%x", item_name, item_offs, old_item_offs);
                    /* set the old item state to obsolete */
                    item_update_state(region, old_item_offs, NVRAM_ITEM_STATE_OBSOLETE);
	...
	SYS_LOG_INF("seg_offset 0x%x, write_offset 0x%x", region->seg_offset,
		region->seg_write_offset);
...
static int region_scan(struct region_info *region)
{
	...
	u32_t offs = 0;
	int err, found = 0;
	int region_old_seg_found = 0;
	int region_old_seg_off = 0;
	int region_old_seg_id = 0;
	int last_region_off = 0;
	int last_region_found = 0;
	...
	while (offs < region->total_size) {
		...
		SYS_LOG_INF("region %x %d seq %d %d %d\n", offs, is_valid_region_seg_header(&hdr), \
			hdr.seq_id, region->seg_seq_id, found);
		...
		if (is_valid_merge_region_seg_header(&hdr) &&
		    (!region_old_seg_found || ((s8_t)(hdr.seq_id - region_old_seg_id)) > 0)) {
			region_old_seg_id = hdr.seq_id;
			region_old_seg_off = offs;
			region_old_seg_found = 1;
		}
	...
	if (found) {
		SYS_LOG_INF("found region %s seg offset 0x%x\n", region->name, region->seg_offset);
		...
		while (offs < region->total_size) {
			...
			if (is_valid_region_seg_header(&hdr) &&
				(((s8_t)(region->seg_seq_id - hdr.seq_id )) == 1)) {
				last_region_found = 1;
				last_region_off = offs;
				SYS_LOG_INF("found old region %d seg offset 0x%x seq %d new %d\n", last_region_found, last_region_off, \
					hdr.seq_id, region->seg_seq_id);
			}

			offs += region->seg_size;
		}

		if(last_region_found){

			SYS_LOG_INF("merge segment from %x to %x\n", last_region_off, region->seg_offset);
			/* init new seg */
			region_init_new_seg(region, region->seg_offset);

			/* copy valid data to new seg */
			region_copy_seg(region, region->seg_offset, last_region_off, region->seg_size, true);

			/* current seg set to obsolete */
			region_seg_update_state(region, last_region_off, NVRAM_REGION_SEG_STATE_OBSOLETE);

		}

		err = region_seg_scan(region, false);
		if (err) {
		...
		if(region_old_seg_found){
			region->seg_seq_id = region_old_seg_id;
			region->seg_offset = region_old_seg_off;

			SYS_LOG_INF("found merge region %s seg offset 0x%x\n", region->name, region->seg_offset);
			err = region_seg_scan(region, true);
			if (err) {
				SYS_LOG_ERR("invalid region seg offset 0x%x\n", region->seg_offset);
				return -1;
			}
			...
			SYS_LOG_INF("region %s: first init\n", region->name);
...
int nvram_config_traverse(uint8_t * data_buf, uint32_t data_len, void *priv_data, void(* name_cb)(uint8_t * buf, uint32_t size, void *priv_data), void(* data_cb)(uint8_t * buf, uint32_t size, void *priv_data))
{
	k_sched_lock();

	region_traverse(&user_nvram_region, data_buf, data_len, priv_data, name_cb, data_cb);
...
int nvram_config_init(struct device *dev)
{
	...
	storage = nvram_user_ext_storage_init();
	if (!storage) {
		SYS_LOG_ERR("NVRAM storage driver was not found!\n");
		return -ENODEV;
	}
...
SYS_INIT(nvram_config_init, PRE_KERNEL_2, 1);
