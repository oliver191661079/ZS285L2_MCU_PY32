...
#define DT_DRV_COMPAT actions_acts_usb
...
#define SYS_LOG_DOMAIN "usb_otg_aotg"
...
int ep_is_busy(uint8_t ep)
{
	uint8_t ep_idx = USB_EP_ADDR2IDX(ep);

	return (usb_read8(INxCS(ep_idx)) & BIT(EPCS_BUSY));
}
/*
				...
				const uint32_t data_len, uint32_t * const ret_bytes)
{
	...
	if (USB_EP_IN_DMA_CAP(ep_idx)) {
#if 0  /* short package use DMA to transfer too. */
...
#else
		/*
		 * The complete callback of DMA does not indicate that the current data transfer
		 * has been completed, and it is necessary to wait until the endpoint FIFO is
		 * empty(cause a interrupt event).
		 */
		usb_write8(INIRQ, BIT(ep_idx));
		usb_set_bit8(INIEN, ep_idx);
#endif
		...
		/* single mode, not autoload */
		usb_write8(IN1_DMACTL, 0x11);

		SYS_LOG_DBG("IN1_DMALEN1L:%x", sys_read32(IN1_DMALEN1L));
		...
		/* 8 channel for all. */
		for(int h=0; h<8; h++){
			SYS_LOG_DBG("%d-DMA0SADDR0:%x", h, sys_read32(DMA0SADDR0+h*0x100));
			SYS_LOG_DBG("%d-DMA0SADDR0:%x", h, sys_read32(DMA0DADDR0+h*0x100));
			SYS_LOG_DBG("%d-DMA0BC:%x", h, sys_read32(DMA0BC+h*0x100));
			SYS_LOG_DBG("%d-DMA0CTL:%x", h, sys_read32(DMA0CTL+h*0x100));
		}
#endif

#if 0   /* DMA burst8 mode. */
		usb_write8(IN1_DMACTL, BIT(DMACTL_FIFORST));
		usb_write8(IN1_DMACTL, 0);
		usb_write8(IN1_DMALEN1L, (uint8_t)len);
		usb_write8(IN1_DMALEN1M, (uint8_t)(len >> 8));
...
int usb_dc_ep_read_actual(uint8_t ep, uint32_t *read_bytes)
{
	...
	num = USB_EP_ADDR2IDX(ep);
	if(num < USB_AOTG_OUT_EP_NUM) {
...
#ifdef CONFIG_USB_AOTG_UDC_DMA
static void usb_aotg_epin_dma_callback(struct device *dev,
...
//	uint8_t ep;
	 ...
	 * The complete callback of DMA does not indicate that the current data transfer
	 * has been completed, and it is necessary to wait until the endpoint FIFO is
	 * empty(cause a interrupt event),So it should be called back to the application
	 * layer from the controller interrupt event after the IN endpoint data has been written
	 */
#if 0
...
static void usb_aotg_epout_dma_callback(struct device *dev,
				uint32_t channel, int reason)
{
	struct aotg_dc_ep_ctrl_prv *ep_ctrl;
...
static int usb_aotg_dma_init(void)
{
	...
	usb_aotg_dma.epout_dma_config_single.dma_slot = DMA_ID_USB0;
	usb_aotg_dma.epout_dma_config_single.complete_callback_en = 1;
	usb_aotg_dma.epout_dma_config_single.head_block = &usb_aotg_dma.epin_dma_block;
...
static inline void aotg_hc_handle_ep0in(void)
{
	...
	case USB_AOTG_SETUP:
		...
		if (urb->ep == USB_CONTROL_IN_EP0) {
			...
			/* Ensure that DATA or STATUS phase is located after the SOF. */
			k_busy_wait(500);
			/* send IN token */
			usb_write8(OUT0BC, 0x0);
...
static inline void aotg_hc_handle_epout_err(uint8_t ep_idx)
{
	...
	SYS_LOG_DBG("ep_idx: %d, type: 0x%x\n", ep_idx, type);
...
static int usb_aotg_pre_init(struct device *dev)
{
	...
	IRQ_CONNECT(USB_AOTG_IRQ, CONFIG_IRQ_PRIO_HIGH,
