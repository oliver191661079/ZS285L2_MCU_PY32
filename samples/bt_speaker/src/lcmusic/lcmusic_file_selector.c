...
static int _iterator_filter_dir(const char *path)
{
	...
	if (strncmp
	    ...
	    || strncmp(app_manager_get_current_app(), APP_ID_SD_MPLAYER,
		       strlen(APP_ID_SD_MPLAYER)) == 0) {
		if (strstr(path, RECORD_DIR) || strstr(path, ALARM_DIR)
		    || strstr(path, SYSTEM_DIR) || strstr(path, LOOP_DIR))
			return true;
		else
			return false;
	} else
...
const char *lcmusic_play_next_url(struct lcmusic_app_t *lcmusic,
				  bool force_switch)
{
	SYS_LOG_INF("state %d", lcmusic->music_state);
	...
	const char *path =
	    iterator_next(iter, force_switch, &lcmusic->music_bp_info.track_no);
...
const char *lcmusic_play_prev_url(struct lcmusic_app_t *lcmusic)
{
	...
	const char *path =
	    iterator_prev(iter, &lcmusic->music_bp_info.track_no);
...
const char *lcmusic_play_next_folder_url(struct lcmusic_app_t *lcmusic)
{
	...
	const char *path =
	    iterator_next_folder(iter, &lcmusic->music_bp_info.track_no);
...
const char *lcmusic_play_prev_folder_url(struct lcmusic_app_t *lcmusic)
{
	...
	const char *path =
	    iterator_prev_folder(iter, &lcmusic->music_bp_info.track_no);
...
const char *lcmusic_play_set_track_no(struct lcmusic_app_t *lcmusic,
				      u16_t track_no)
{
	SYS_LOG_INF("state %d, track %d", lcmusic->music_state, track_no);
...
{
	SYS_LOG_INF("state %d mode %d", lcmusic->music_state, mode);
...
int lcmusic_get_cluster_by_url(struct lcmusic_app_t *lcmusic)
{
	...
	memset(lcmusic->music_bp_info.file_dir_info, 0,
	       sizeof(lcmusic->music_bp_info.file_dir_info));
			...
			if (res) {
				...
				do {
					...
					if (res || entry->name[0] == 0) {
						SYS_LOG_ERR
						    ("fs_readdir failed (res=%d), skip this\n",
						     res);
						fs_closedir(zdp);
						res = -EINVAL;
						goto exit;
...
int lcmusic_get_url_by_cluster(struct lcmusic_app_t *lcmusic)
{
	...
	if (i == 0) {
		...
		memset(&lcmusic->music_bp_info, 0,
		       sizeof(lcmusic->music_bp_info));
		goto exit;
	}
	strcpy(lcmusic->cur_url, lcmusic->cur_dir);
	if (lcmusic->cur_url[strlen(lcmusic->cur_url) - 1] != ':'
	    && lcmusic->cur_url[strlen(lcmusic->cur_url) - 1] != '/')
	...
	do {
		...
		res = fs_opendir_cluster(zdp, lcmusic->cur_dir,
					 lcmusic->music_bp_info.
					 file_dir_info[i].cluster,
					 lcmusic->music_bp_info.
					 file_dir_info[i].dirent_blk_ofs);
		...
		if (!res) {
			...
			if (entry->type == FS_DIR_ENTRY_FILE
			    && _iterator_match_fn(entry->name, 0)) {
				strcpy(lcmusic->cur_url +
				       strlen(lcmusic->cur_url), entry->name);
				...
				if (dir_layer > i
					...
					res = -EINVAL;
					SYS_LOG_INF
					    ("bp no match layer(%d),i(%d),file_size(%d)\n",
					     dir_layer, i, file_size);
					goto exit;
				}
			} else if (entry->type == FS_DIR_ENTRY_DIR
				   && _iterator_match_fn(entry->name, 1)) {
				strcpy(lcmusic->cur_url +
				       strlen(lcmusic->cur_url), entry->name);
				lcmusic->cur_url[strlen(lcmusic->cur_url)] =
				    '/';
...
static u8_t _lcmusic_validate_bp_info(struct lcmusic_app_t *lcmusic)
{
	int res = -EINVAL;
	fs_dir_t *zdp = NULL;
	struct fs_dirent *entry = NULL;
	u8_t format = UNSUPPORT_TYPE;
	...
	if (lcmusic->music_bp_info.file_dir_info[0].cluster ||
		...
		res = fs_opendir_cluster(zdp, lcmusic->cur_dir,
					 lcmusic->music_bp_info.
					 file_dir_info[0].cluster,
					 lcmusic->music_bp_info.
					 file_dir_info[0].dirent_blk_ofs);
		if (!res) {
			res = fs_readdir(zdp, entry);
			if (res || entry->name[0] == 0) {
			...
			if (entry->type != FS_DIR_ENTRY_FILE
			    ...
			    || (u32_t) (entry->size) !=
			    lcmusic->music_bp_info.file_size) {
				memset(&lcmusic->music_bp_info, 0x0, sizeof(struct music_bp_info_t));
				SYS_LOG_ERR("bp unmatch, clear bp info\n");
			}
			format = get_music_file_type(entry->name);
...
int lcmusic_get_url_by_cluster(struct lcmusic_app_t *lcmusic)
{
	...
	format = _lcmusic_validate_bp_info(lcmusic);
	if (UNSUPPORT_TYPE != format) {
		/*get file path */
		memset(lcmusic->cur_url, 0, sizeof(lcmusic->cur_url));
		snprintf(lcmusic->cur_url, sizeof(lcmusic->cur_url),
			 "%s%s/%s%u/%u/%u/%s", OPEN_MODE, lcmusic->cur_dir,
			 CLUSTER,
			 lcmusic->music_bp_info.file_dir_info[0].cluster,
			 lcmusic->music_bp_info.file_dir_info[0].dirent_blk_ofs,
			 lcmusic->music_bp_info.file_size, get_music_type_str(format));
...
#if CONFIG_BKG_SCAN_DISK
static void _lcmusic_scan_disk_thread(void *parama1, void *parama2,
				      void *parama3)
{
	struct lcmusic_app_t *lcmusic = lcmusic_get_app();
...
/* create a thread to scan disk*/
	os_thread_create(scan_disk_stack, SCAN_DISK_STACKSIZE,
			 ...
			 NULL, NULL, NULL, CONFIG_APP_PRIORITY - 1, 0, 0);
#else
	_lcmusic_scan_disk_start();
#endif
}
