...
static int _btcall_ring_get_file_name(u8_t num, u8_t * file_name,
				      u8_t file_name_len)
{
	if ((num >= '0') && (num <= '9')) {
...
void btcall_ring_play_next(void)
{
	...
	// Early exit if the ring manager is not available
	if (ring_manager == NULL) {
		return;
	}
	...
	// Early exit if ring info is invalid or the state is done
	if (ring_info == NULL || ring_manager->state == STATE_RING_DONE) {
		return;
	}

#ifdef CONFIG_BT_CALL_FORCE_PLAY_PHONE_NUM
	struct btcall_app_t *btcall = btcall_get_app();
	bool is_last_phone_num = (ring_info->current_index == (ring_info->phone_num_cnt - 1));

	// Check if it's the last phone number and if the call is established
	if (is_last_phone_num) {
	...
	ring_info->current_index = (ring_info->current_index + 1) % ring_info->phone_num_cnt;

	// Retrieve the file name for the next phone number and play it
...
static void _btcall_ring_start(struct thread_timer *ttimer, void *expiry_fn_arg)
{
	...
	_btcall_ring_get_file_name(ring_manager->
				   ring_info->phone_num[ring_manager->
							ring_info->current_index],
...
int btcall_ring_manager_init(void)
{
	...
	thread_timer_init(&ring_manager_context->call_ring_timer,
			  _btcall_ring_start, NULL);
